...
//   ../../ui/... (from packages/ui-docs/src/foo.ts)
//   ../../../ui/... (from packages/ui-docs/src/deep/foo.ts)
//
 ...
 * like packages (e.g., ./ui/ or ../ui/ within the same package).
 */
...
function getImportSource(node) {
  // Static import: import x from "../ui/foo"
  if (node.source && node.source.type === "Literal") {
    return node.source.value;
  }
  // Dynamic import: import("../ui/foo")
  if (
...
export const noCrossPackageImportsRule = defineRule({
  meta: {
    ...
    messages: {
      [RULE_NAME]:
        "Cross-package relative import detected. Import from '{{packageName}}' using its package name (e.g., '@aolda/{{packageName}}') instead of relative paths ('{{importPath}}').",
    },
    schema: [],
  },
  defaultOptions: [],
  createOnce(context) {
    ...
    return {
      // Static imports: import x from "../ui/foo"
      ...
      // export from: export { x } from "../ui/foo"
