...
/**
 * Enforces the Aolda variant export standard for components:
 * - AOLDA_{COMPONENT}_VARIANTS (required)
 * - AOLDA_{COMPONENT}_DEFAULT_VARIANTS (required)
 * - AOLDA_{COMPONENT}_BASE_STYLES (optional, but must have AOLDA_ prefix)
 *
...
function validateExportName(exportName, componentName) {
  const variantsPattern = `AOLDA_${componentName}_VARIANTS`;
  const defaultVariantsPattern = `AOLDA_${componentName}_DEFAULT_VARIANTS`;
  const baseStylesPattern = `AOLDA_${componentName}_BASE_STYLES`;
  ...
  if (exportName.endsWith("_VARIANTS") && exportName.startsWith("AOLDA_")) {
    return {
      valid: false,
  ...
  if (exportName.endsWith("_BASE_STYLES")) {
    // BASE_STYLES must have AOLDA_ prefix
    if (!exportName.startsWith("AOLDA_")) {
      return {
        valid: false,
...
export const enforceVariantStandardRule = defineRule({
  meta: {
    ...
    docs: {
      description:
        "Enforce Aolda variant standard: AOLDA_{COMPONENT}_VARIANTS, AOLDA_{COMPONENT}_DEFAULT_VARIANTS, and optionally AOLDA_{COMPONENT}_BASE_STYLES",
    },
    messages: {
      incorrectName:
        "Export name '{{actual}}' should be '{{expected}}' to follow Aolda variant naming convention",
      missingVariants:
        "Component must export AOLDA_{{component}}_VARIANTS. Found: {{found}}",
      missingDefaultVariants:
        "Component must export AOLDA_{{component}}_DEFAULT_VARIANTS. Found: {{found}}",
    },
    schema: [],
  },
  defaultOptions: [],
  createOnce(context) {
    ...
    return {
      ...
      "Program:exit"() {
        ...
        const expectedVariants = `AOLDA_${componentName}_VARIANTS`;
        const expectedDefaultVariants = `AOLDA_${componentName}_DEFAULT_VARIANTS`;
