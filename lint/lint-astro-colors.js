 ...
 * 2. Invalid/unknown semantic tokens not defined in theme-aolda.css
 *
 * Usage:
 *   node lint/lint-astro-colors.js [directory]
 *   node lint/lint-astro-colors.js packages/ui-docs/src
 *
 * Exit codes:
 *   0 - No issues found
 *   1 - Issues found
...
const VALID_AOLDA_SEMANTIC_COLORS = new Set([
  ...VALID_COLOR_TOKENS,
  ...
  for (const { value, line } of classAttrs) {
    ...
    if (primitive) {
      issues.push({
        ...
        message: `Avoid Tailwind primitive color '${primitive.token}'. Use Aolda semantic tokens instead.`,
      });
      continue;
    }
    ...
    if (invalid) {
      issues.push({
        ...
        message: `Invalid color token '${invalid.token}'. Token '${invalid.tokenName}' is not defined in theme-aolda.css.`,
      });
    }
  }
...
function main() {
  ...
  const targetDir = args[0] || "packages/ui-docs/src";
  ...
  for (const { path, issues } of fileIssues) {
    for (const issue of issues) {
      ...
      console.log(
        `  ${symbol} aolda/${issue.type === "primitive" ? "no-primitive-colors" : "invalid-color-token"}: ${issue.message}`,
      );
