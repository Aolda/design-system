...
// We want to enforce use of Aolda semantic color tokens `--color-aolda-*`.
// Any Tailwind color utility (e.g. `bg-blue-500`) or legacy semantic
// utility (e.g. `bg-aolda-hairline`, `text-aolda-default`) in class strings should be
...
function parseAoldaSemanticColors() {
  const themeFiles = [
    resolve(__dirname, "../packages/ui/src/styles/theme-aolda.css"),
    resolve(__dirname, "../packages/ui/src/styles/theme-fedramp.css"),
  ];
...
// Valid Aolda semantic color tokens derived from theme CSS files.
...
} = parseAoldaSemanticColors();

// Legacy export for backwards compatibility
export const VALID_AOLDA_SEMANTIC_COLORS = new Set([
  ...VALID_COLOR_TOKENS,
...
function findPrimitiveColor(str) {
  ...
  while ((match = TOKEN_RE.exec(str))) {
    ...
    // Skip valid Aolda semantic color tokens (e.g. bg-surface, text-secondary,
    // border-aolda-fill). These are backed by theme-aolda.css custom properties.
    if (VALID_AOLDA_SEMANTIC_COLORS.has(colorFamily)) continue;

    // Flag aolda- prefixed classes (e.g. text-aolda-surface).
    // These are invalid; use semantic tokens directly (e.g. text-aolda-default).
    if (colorFamily.startsWith("aolda-"))
    ...
    // Only flag if it's a Tailwind primitive AND not a valid Aolda semantic token.
    // This handles cases like "green-2" where "green" is a Tailwind primitive
    // but "green-2" is a valid Aolda semantic token.
    if (
      TAILWIND_COLOR_FAMILIES.has(primitiveFamily) &&
      !VALID_AOLDA_SEMANTIC_COLORS.has(colorFamily)
    )
      return { type: "primitive", token: fullToken };
  }
...
export const noPrimitiveColorsRule = defineRule({
  meta: {
    ...
    messages: {
      [RULE_NAME]:
        "Avoid Tailwind color utilities (e.g. `bg-blue-500`, `border-red-500`). Use Aolda semantic tokens instead.",
      [INVALID_TOKEN_RULE]:
        "Invalid color token '{{token}}'. Token '{{tokenName}}' is not defined in theme-aolda.css or theme-fedramp.css.",
    },
    schema: [],
  },
  defaultOptions: [],
  createOnce(context) {
    ...
    return {
      ...
      Property(node) {
        ...
        if (name !== "classes" && name !== "className") return;
