...
/**
 * Validates that a changeset exists for the @aolda/ui package
 * when files in packages/ui/ are modified in a pull request.
 */

const AOLDA_PACKAGE_NAME = "@aolda/ui";
const AOLDA_PATH = "packages/ui";
...
function main() {
  console.log(
    `🔍 Validating changeset for aolda package: ${AOLDA_PACKAGE_NAME}`,
  );
  ...
  // design, their diff modifies `packages/ui/` (version bump + CHANGELOG)
  ...
  // Check if aolda files have been modified
  const hasAoldaChanges = checkForAoldaChanges();
  if (!hasAoldaChanges) {
    console.log(
      "No changes detected in packages/ui/, skipping changeset validation",
    );
    return;
  }

  console.log("Changes detected in packages/ui/");

  // Check for newly added changesets in this MR
  const newChangesets = getNewlyAddedChangesets();
  const newAoldaChangesets = newChangesets.filter((cs) =>
    cs.packages.includes(AOLDA_PACKAGE_NAME),
  );

  if (newAoldaChangesets.length === 0) {
    ...
    if (newChangesets.length === 0) {
      console.error(
        "\x1b[31;1m❌ ERROR: Changes detected in packages/ui/ but no NEW changeset files found\x1b[0m",
      );
    } else {
      console.error(
        "\x1b[31;1m❌ ERROR: Found NEW changeset files, but none target @aolda/ui\x1b[0m",
      );
      console.error("");
    ...
    console.error('   2. Select "\x1b[36m@aolda/ui\x1b[0m" when prompted');
    console.error(
    ...
    console.error("");
    console.error(
      "This ensures proper versioning and changelog generation for the aolda package.",
    );
    console.error("");
    if (process.env.CI) {
      console.error(
  ...
  console.log(
    `✅ Found ${newAoldaChangesets.length} NEW changeset(s) for @aolda/ui:`,
  );
  newAoldaChangesets.forEach((cs) => {
    console.log(`   - ${cs.name}`);
  });
...
function checkForAoldaChanges(): boolean {
  const result = hasChangesInPath(AOLDA_PATH);

  if (result === null) {
    console.warn(
      "⚠️  Warning: Could not determine if aolda changes exist, assuming they do",
    );
    return true;
  }
...
function getNewlyAddedChangesets(): ChangesetFile[] {
  // Determine working directory (handle both repo root and packages/ui contexts)
  const cwd = process.cwd().includes("packages/ui") ? "../.." : ".";
  ...
  for (const { status, path: filePath } of newFiles) {
    ...
    try {
      ...
      const repoRoot = process.cwd().includes("packages/ui") ? "../.." : ".";
