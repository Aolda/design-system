...
```tsx
import { DatePicker, type DateRange } from "@aolda/ui";
```
...
import {
  DatePicker,
  type DateRange,
} from "@aolda/ui/components/date-picker";
```
...
import { DatePicker } from "@aolda/ui";
...
<p>
  Use `variant="primary"` for brand-colored selections, or
  `variant="neutral"` for high-contrast neutral selections.
</p>
<ComponentExample demo="DatePickerVariantDemo">
  <DatePickerVariantDemo client:load />
</ComponentExample>
...
```tsx
import { useState } from "react";
import { DatePicker, Popover, Button } from "@aolda/ui";
...
## API Reference
  ...
  <ul class="mt-4 list-disc list-inside space-y-2">
    ...
    <li>`variant` — <code class="text-xs">"primary" | "neutral"</code> — Visual tone for selected dates and range endpoints</li>
  ...
  <p class="mt-4 text-aolda-subtle text-sm">
...
### Differences from react-day-picker
  ...
  <p>
    For consistency with other Aolda form components, DatePicker uses `onChange` instead of react-day-picker's `onSelect`.
