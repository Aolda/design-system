...
<ComponentSection>
  <div class="mb-4 rounded-md border border-aolda-brand/20 bg-aolda-brand/5 p-4">
    <p class="text-sm text-aolda-strong">
      <strong>New in v1.10:</strong> Shiki-powered syntax highlighting with lazy
      loading. Import from `@aolda/ui/code` to use.
    </p>
  </div>
...
## Overview
  ...
  <p>
    ...
    entry point ( `@aolda/ui/code` ) to avoid bundling Shiki for apps
    that don't need it.
  </p>
</ComponentSection>
...
```
  ...
  <div class="mt-4 rounded-md border border-aolda-warning/20 bg-aolda-warning/5 p-4">
    <p class="text-sm text-aolda-strong">
      <strong>Important:</strong> Do NOT import from the main `@aolda/ui` entry.
...
<p>
  Customize the highlight color with the `--aolda-code-highlight-bg` CSS
  variable.
</p>
...
## Themes
  ...
  <p>
    CodeHighlighted uses hardcoded themes for consistent styling across all Aolda
    applications:
  </p>
  ...
  <p class="text-sm text-aolda-subtle">
...
import { highlightCode } from "@aolda/ui/code/server";
...
import { createServerHighlighter } from "@aolda/ui/code/server";
...
import { ShikiProvider } from "@aolda/ui/code";
...
## Bundle Size
  ...
  <div class="overflow-x-auto">
    <table class="w-full text-sm">
      ...
      <tbody>
        <tr class="border-b border-aolda-hairline/50">
  ...
  <p class="mt-4 text-sm text-aolda-subtle">
    Teams that don't import from `@aolda/ui/code` pay 0 KB.
  </p>
</ComponentSection>
...
```tsx
// Before (deprecated)
import { Code, CodeBlock } from "@aolda/ui";
