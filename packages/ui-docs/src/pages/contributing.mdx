...
title: "Contributing"
description: "Learn how to contribute to Aolda, from setup and workflows to PR and release guidelines. Everything you need for setup, development, quality checks, and PR process is documented here."
---
...
Internal contributors normally clone and push directly to `aolda/ui`:

```bash
git clone https://github.com/Aolda/design-system.git
git switch -c <branch-name>
git push -u origin <branch-name>
```

If you do not have write access, contact your manager or Aolda maintainers.
...
- **Component**: reusable UI primitive in `packages/ui/src/components/`
- **Docs only**: docs/demo/navigation updates in `packages/ui-docs/`
...
```bash
# Terminal 1: watch Aolda package output
pnpm --filter @aolda/ui dev
...
1. Build the feature in `packages/ui/src/...`.
2. Add or update demos in `packages/ui-docs/src/components/demos/`.
3. Add/update tests in `packages/ui`.
4. Add or update docs pages in `packages/ui-docs/src/pages/`.
...
- Follow existing Aolda patterns for variants, props, and structure.
...
If your change touches `packages/ui/` and should ship, add a changeset:
...
- Use PR title format: `[package] short description` (example: `[aolda] add meter warning variant`).
...
- Most Aolda tests are Vitest-based and live under `packages/ui`.
...
- Use semantic Aolda tokens and avoid raw Tailwind color classes.
...
- Do not skip changesets for releasable `packages/ui` changes.

---

## Related Docs

Follow the [changeset guide](https://github.com/Aolda/design-system/blob/main/.changeset/README.md) to document releasable library changes and see the [AGENTS.md](https://github.com/Aolda/design-system/blob/main/packages/ui/AGENTS.md) for conventions and patterns.
