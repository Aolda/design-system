...
>(function ExpandableNode({ title, children, ...props }, ref) {
  ...
  return (
    <li
      ref={ref}
      {...props}
      className="rounded-lg shadow bg-aolda-base ring ring-aolda-hairline overflow-hidden"
    >
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full cursor-pointer items-center justify-between gap-2 px-3 py-2 text-sm font-medium text-aolda-default"
      >
        {title}
        <CaretDownIcon
          className={`size-4 text-aolda-subtle transition-transform ${open ? "rotate-180" : ""}`}
        />
      </button>
      {open && (
        <div className="border-t border-aolda-hairline px-3 py-2 text-sm text-aolda-subtle">
          {children}
        </div>
      )}
    </li>
...
export function FlowCustomContentDemo() {
  return (
    <Flow>
      <Flow.Node
        render={<li className="rounded-full size-4 bg-aolda-hairline" />}
      />
      <Flow.Node
        render={
          <li className="bg-aolda-contrast text-aolda-inverse rounded-lg font-medium py-2 px-3">
            my-worker
          </li>
        }
      />
...
export function FlowAnchorDemo() {
  return (
    <Flow>
      ...
      <Flow.Node
        render={
          <li className="shadow-none rounded-lg ring ring-aolda-hairline bg-aolda-overlay">
            <Flow.Anchor
              type="end"
              render={
                <div className="text-aolda-subtle h-10 flex items-center px-2.5">
                  my-worker
                </div>
              }
            />
            <Flow.Anchor
              type="start"
              render={
                <div className="bg-aolda-base rounded ring ring-aolda-hairline shadow px-2 py-1.5 m-1.5 mt-0">
                  Bindings
                  <span className="text-aolda-subtle w-5 ml-3">2</span>
                </div>
              }
            />
          </li>
...
export function FlowCenteredDemo() {
  return (
    <Flow align="center">
      ...
      <Flow.Node
        render={
          <li className="py-6 px-3 rounded-md shadow bg-aolda-base ring ring-aolda-hairline">
            Taller node
          </li>
        }
      />
...
export function FlowPanningDemo() {
  return (
    <Flow className="rounded-lg border border-aolda-hairline">
      <Flow.Node>Start</Flow.Node>
...
export function FlowSidebarBugDemo() {
  ...
  return (
    <div className="relative overflow-hidden rounded-lg ring ring-aolda-hairline bg-aolda-base min-h-64 flex flex-col">
      {/* Toolbar */}
      <div className="flex items-center gap-2 border-b border-aolda-hairline px-3 py-2 shrink-0">
        <button
          type="button"
          onClick={() => setSidebarOpen((v) => !v)}
          className="flex items-center gap-1.5 rounded px-2 py-1 text-xs font-medium text-aolda-default hover:bg-aolda-elevated transition-colors"
        >
          <SidebarSimpleIcon className="size-4" />
          {sidebarOpen ? "Close sidebar" : "Open sidebar"}
        </button>
        <span className="text-xs text-aolda-subtle">
      ...
      <div className="flex flex-1 min-h-0">
        {/* Sidebar */}
        <div
          className="shrink-0 overflow-hidden transition-[width] duration-300 border-r border-aolda-hairline bg-aolda-elevated"
          style={{ width: sidebarOpen ? 160 : 0 }}
        >
          <div className="w-40 p-3 space-y-1">
            <p className="text-xs font-semibold text-aolda-subtle uppercase tracking-wide mb-2">
              Sidebar
            </p>
            {["Overview", "Settings", "Logs", "Analytics"].map((item) => (
              <div
                key={item}
                className="rounded px-2 py-1 text-sm text-aolda-default hover:bg-aolda-base cursor-default"
              >
                {item}
              </div>
            ))}
