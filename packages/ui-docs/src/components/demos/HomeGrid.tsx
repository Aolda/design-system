...
function ToastTriggerButton() {
  const toastManager = useAoldaToastManager();
  return (
    <Button
      onClick={() =>
        toastManager.add({
...
export function HomeGrid() {
      ...
      Component: (
        ...
        >
          ...
          <Collapsible.DefaultPanel>
            Aolda is Aolda's component library.
          </Collapsible.DefaultPanel>
        </Collapsible.Root>
      ),
    },
  ...
  return a + b;
    ...
    },
    {
      ...
      Component: (
        <div className="flex items-center gap-1 text-sm">
          <span className="text-aolda-subtle">Home</span>
          <span className="text-aolda-inactive">/</span>
          <span className="text-aolda-subtle">Docs</span>
          <span className="text-aolda-inactive">/</span>
    ...
    },
    {
      name: "ClipboardText",
      id: "clipboard-text",
      Component: <ClipboardText text="pnpm add @aolda/ui" />,
    },
    {
      name: "CommandPalette",
      id: "command-palette",
    ...
    },
    {
      ...
      Component: (
        <Grid variant="side-by-side" gap="sm" className="w-[140px]">
          <GridItem className="rounded bg-aolda-control p-3 text-center text-xs">
            1
          </GridItem>
          <GridItem className="rounded bg-aolda-control p-3 text-center text-xs">
            2
  ...
  return (
    <ul className="grid auto-rows-min grid-cols-1 gap-px bg-aolda-hairline md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
      {components.map((c) => {
        const route = componentRoutes[c.id] || null;
        return (
          <li
            className="relative flex aspect-square items-center justify-center bg-aolda-canvas"
            key={c.name}
          >
            {route ? (
              <a
                href={route}
                className="absolute top-4 left-4 text-base font-medium text-aolda-subtle hover:text-aolda-default"
              >
                {c.name}
              </a>
            ) : (
              <span className="absolute top-4 left-4 text-base font-medium text-aolda-subtle italic">
                {c.name}
              </span>
            )}
            <div className="flex w-full items-center justify-center p-8 tracking-normal leading-normal">
              {c.Component ?? (
                <p className="text-base font-medium text-aolda-subtle">TBD</p>
              )}
            </div>
          </li>
        );
