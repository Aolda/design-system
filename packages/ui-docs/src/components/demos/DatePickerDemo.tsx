...
import { DatePicker, Popover, Button, type DateRange } from "@aolda/ui";
...
export function DatePickerSingleDemo() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      />
      <p className="text-sm text-aolda-subtle">
  ...
  return (
    <div className="flex flex-col gap-4">
      <DatePicker
        ...
        max={7}
        footer={
          <span className="text-xs text-aolda-subtle">Select 3-7 nights</span>
        }
      />
    </div>
  );
...
/**
 * Brand and neutral visual variants.
 */
export function DatePickerVariantDemo() {
  const [primaryDate, setPrimaryDate] = useState<Date | undefined>();
  const [neutralDate, setNeutralDate] = useState<Date | undefined>();

  return (
    <div className="grid gap-6 md:grid-cols-2">
      <div className="flex flex-col gap-3">
        <p className="text-sm font-medium text-aolda-default">Primary</p>
        <DatePicker
          mode="single"
      ...
      <div className="flex flex-col gap-3">
        <p className="text-sm font-medium text-aolda-default">Neutral</p>
        <DatePicker
          mode="single"
          selected={neutralDate}
          onChange={setNeutralDate}
          variant="neutral"
        />
      </div>
    </div>
  ...
  return (
    <Popover>
      ...
      <Popover.Content className="p-0">
        <div className="flex">
          <div className="flex flex-col gap-1 border-r border-aolda-hairline p-2 text-sm">
            {presets.map((preset) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-md px-3 py-1.5 text-left whitespace-nowrap ${
                    isActive
                      ? "bg-aolda-bg-inverse text-aolda-text-inverse"
                      : "text-aolda-strong hover:bg-aolda-control"
                  }`}
                >
                  {preset.label}
                </button>
...
export function DatePickerDisabledWithFooterDemo() {
  ...
  return (
    <DatePicker
      ...
      fixedWeeks
      footer={
        <p className="text-xs text-aolda-subtle pt-2 w-full">
