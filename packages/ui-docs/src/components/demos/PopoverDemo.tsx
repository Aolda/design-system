...
import { Popover, Button } from "@aolda/ui";
    ...
    <Popover>
      ...
      <Popover.Content className="w-64">
        <div className="flex items-center gap-3">
          <div className="size-10 rounded-full bg-aolda-recessed" />
          <div>
            <Popover.Title>Jane Doe</Popover.Title>
            <p className="text-sm text-aolda-subtle">jane@example.com</p>
          </div>
        </div>
        <div className="mt-3 flex gap-2 border-t border-aolda-hairline pt-3">
...
export function PopoverVirtualAnchorDemo() {
  ...
  return (
    <div className="w-full">
      <div className="overflow-hidden rounded-lg border border-aolda-hairline">
        <table className="w-full text-sm">
          <thead className="bg-aolda-elevated">
            <tr>
          ...
          <tbody className="divide-y divide-aolda-hairline">
            {rows.map((row) => (
              <tr
                key={row.id}
                ref={(el) => {
                ...
                className={
                  selectedRow === row.id ? "bg-aolda-recessed" : "bg-aolda-base"
                }
              >
                <td className="px-4 py-2 font-mono">{row.name}</td>
                <td className="px-4 py-2 text-aolda-subtle">{row.status}</td>
