...
import { SensitiveInput, Button } from "@aolda/ui";
    ...
    <div className="flex flex-col gap-4">
      {sizes.map((size) => (
        <div key={size} className="flex items-center gap-2">
          <span className="w-12 text-sm text-aolda-subtle">{size}</span>
          <SensitiveInput
            label={`${size} size`}
            size={size}
...
export function SensitiveInputControlledDemo() {
  ...
  return (
    <div className="flex w-80 flex-col gap-4">
      ...
      />
      <div className="text-sm text-aolda-subtle">
        Current value: <code className="text-aolda-default">{value}</code>
      </div>
