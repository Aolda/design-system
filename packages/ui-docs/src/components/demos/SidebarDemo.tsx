import { Sidebar, useSidebar, DropdownMenu } from "@aolda/ui";
import {
  HouseIcon,
  GlobeIcon,
  GearIcon,
...
function DemoContainer({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative h-[540px] w-full overflow-hidden rounded-lg border border-aolda-hairline bg-aolda-base">
      {children}
    </div>
  );
}
...
function DemoMain({ children }: { children?: React.ReactNode }) {
  return (
    <main className="flex flex-1 flex-col items-center justify-center gap-2 p-8 text-aolda-subtle">
...
function BrandLogo() {
  return (
    <div className="flex w-full min-w-0 items-center gap-2 px-3 group-data-[state=collapsed]/sidebar:px-2 transition-[padding] duration-250 ease-[cubic-bezier(0.77,0,0.175,1)]">
      <div className="size-4 shrink-0 rounded bg-aolda-brand" />
      <span className="text-sm font-semibold text-aolda-strong truncate group-data-[state=collapsed]/sidebar:hidden">
        Acme Inc
      </span>
    </div>
  );
    ...
    <DropdownMenu>
      <DropdownMenu.Trigger
        render={
          <button
            type="button"
            className="flex w-full min-w-0 items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-medium text-aolda-default hover:bg-aolda-tint focus-visible:ring-1 focus-visible:ring-aolda-hairline outline-none transition-[color,background-color,padding] duration-250 ease-[cubic-bezier(0.77,0,0.175,1)] group-data-[state=collapsed]/sidebar:px-2"
          >
            <active.icon
              className="size-4 shrink-0 text-aolda-brand"
              weight="fill"
            />
            <span className="flex-1 truncate text-left font-semibold text-aolda-strong group-data-[state=collapsed]/sidebar:hidden">
              {active.name}
            </span>
            <CaretUpDownIcon className="size-4 shrink-0 text-aolda-subtle group-data-[state=collapsed]/sidebar:hidden" />
          </button>
        }
      />
      <DropdownMenu.Content className="w-[var(--anchor-width)]">
        {accounts.map((account) => (
          ...
          >
            <account.icon className="size-4 text-aolda-brand" weight="fill" />
            {account.name}
...
function ToggleButton() {
  ...
  return (
    <button
      ...
      className="rounded-lg border border-aolda-hairline bg-aolda-base px-3 py-1.5 text-sm text-aolda-default transition-colors hover:bg-aolda-tint"
    >
