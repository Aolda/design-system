...
import { aoldaColors, type ColorToken } from "virtual:aolda-colors";
...
const ColorSwatch: FC<{ label: string; value: string }> = ({
  ...
  return (
    <div className="flex min-w-0 items-start gap-2">
      <span
        className="inline-flex h-8 w-8 shrink-0 rounded border border-aolda-fill"
        style={{ background: value }}
      />
      <div className="flex min-w-0 flex-col text-xs text-aolda-default">
        ...
        <span className="text-[10px] leading-tight break-normal opacity-60">
          {value}
          {hex && (
            <span className="ml-1 font-mono font-medium text-aolda-default">
              {hex}
            </span>
          )}
        </span>
...
function getTheme(): string {
  return document.body.getAttribute("data-theme") ?? "aolda";
}

function useCurrentTheme(): string {
  return useSyncExternalStore(subscribeToTheme, getTheme, () => "aolda");
}
...
const componentNames: Set<string> = new Set(
  Object.keys(aoldaRegistryJson.components).map((n) => n.toLowerCase()),
);

/**
 * Extract the component name from a token, if it matches a known component.
 *
 * Token format: `--color-aolda-{component}-{variant}` or `--text-color-aolda-{component}-{variant}`
...
function getComponentFromToken(tokenName: string): string | null {
  ...
  // "--color-aolda-badge-red" → "aolda-badge-red"
  // "--text-color-aolda-badge-red-subtle" → "aolda-badge-red-subtle"
  const semantic = tokenName
    .replace(/^--text-color-/, "")
    .replace(/^--color-/, "");

  // Must start with "aolda-"
  if (!semantic.startsWith("aolda-")) return null;

  // "aolda-badge-red" → "badge-red" → check if "badge" is a component
  const afterAolda = semantic.slice("aolda-".length);
  const segments = afterAolda.split("-");

  // Require at least 2 segments: component name + variant.
  // This avoids matching standalone semantic tokens like "aolda-link" or "aolda-surface".
...
function getColorsForTheme(theme: string): ColorsByCategory {
  ...
  const componentGroups: ComponentColorGroup[] = [
    ...
    .map(([comp, tokens]) => {
      ...
      const registryName = Object.keys(aoldaRegistryJson.components).find(
...
const TokenGrid: FC<{ tokens: ColorToken[] }> = ({ tokens }) => (
  <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-2">
    {tokens.map((token: ColorToken) => (
      <div
        key={token.name}
        className={`flex min-w-0 items-center gap-3 rounded-md border bg-aolda-base px-3 py-2 text-xs ${
          token.tokenType === "global"
            ? "border border-aolda-info ring-1 ring-aolda-info/30"
            : "border-aolda-fill"
        }`}
      >
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {token.tokenType === "global" && (
              <span className="rounded bg-aolda-info/20 px-1.5 py-0.5 text-[10px] font-medium text-aolda-link">
                global
              </span>
            )}
          </div>
...
export const TailwindColorTokens: FC = () => {
  ...
  return (
    <div className="flex flex-col gap-6 text-aolda-default">
      <div className="flex flex-col gap-1">
        <h2 className="m-0 text-2xl font-semibold">Colors</h2>
        <div className="text-sm text-aolda-default">
          Displaying {allTokens.length + componentTokenCount} tokens
          {overrideCount > 0 && (
            <span className="ml-1">
              — {overrideCount} overridden by{" "}
              <code className="rounded bg-aolda-brand p-1">{currentTheme}</code>
            </span>
          )}
        </div>
      </div>
        ...
        <section className="flex flex-col gap-3">
          ...
          {componentGroups.map((group) => (
            <div key={group.component} className="flex flex-col gap-3">
              <h3 className="!m-0 text-xs font-semibold text-aolda-subtle">
