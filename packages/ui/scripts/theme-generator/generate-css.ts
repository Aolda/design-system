/**
 * CSS Generator for Aolda Themes
 *
...
function pushModeScopedBaseVariables(
  ...
  const lightSelector =
    themeName === "aolda" ? `:root, ${themeSelector}` : `${themeSelector}`;
  const darkSelector =
    themeName === "aolda"
...
/**
 * Generate the base aolda theme CSS
 * This contains all tokens with their aolda theme values
 */
export function generateAoldaThemeCSS(
  config: ThemeConfig,
  useNewNames = false,
): string {
  ...
  for (const [tokenName, def] of Object.entries(config.text)) {
    const name = useNewNames ? def.newName : tokenName;
    const aoldaColors = def.theme.aolda;

    lines.push(
      `  --text-color-${name}: ${lightDark(aoldaColors.light, aoldaColors.dark)};`,
    );
    lines.push("");
  }
  ...
  for (const [tokenName, def] of Object.entries(config.color)) {
    ...
    lines.push(
      `  --color-${name}: ${lightDark(aoldaColors.light, aoldaColors.dark)};`,
    );
    lines.push("");
  }
  ...
  pushModeScopedBaseVariables(lines, config, "aolda", useNewNames, {
 ...
 * Only includes tokens that differ from the base aolda theme
 */
export function generateThemeOverrideCSS(
  ...
  if (themeName === "aolda") {
    throw new Error("Use generateAoldaThemeCSS for the base aolda theme");
  }
...
export function generateAllThemes(
  ...
  // Always generate base aolda theme
  if (themes.includes("aolda")) {
    files.set(
      "theme-aolda.css",
      generateAoldaThemeCSS(THEME_CONFIG, useNewNames),
    );
  }
