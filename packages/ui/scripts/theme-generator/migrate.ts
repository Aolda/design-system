#!/usr/bin/env node
/**
 * Aolda Token Migration Tool
 *
 * Complete migration workflow:
 * 1. Renames Tailwind classes in source files (bg-surface -> bg-aolda-base)
 * 2. Updates config.ts: newName becomes key, newName field becomes ""
 * 3. Regenerates theme CSS with new names
 *
 * Usage:
 *   pnpm migrate:tokens              # Migrate aolda codebase
...
function rewriteConfig(renameMap: TokenRenameMap): string {
  ...
  lines.push(`export const AVAILABLE_THEMES = ["aolda", "fedramp"] as const;`);
...
function parseArgs(
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--path" && args[i + 1]) {
      ...
      updateConfig = false; // External projects don't update aolda's config
    } else if (arg === "--dry-run") {
      dryRun = true;
...
1. Updates class names in source files (bg-surface -> bg-aolda-base)
...
EXAMPLE - Renaming "surface" to "aolda-base":

  Before config.ts:
    surface: { newName: "aolda-base", theme: { ... } }

  After migration:
    "aolda-base": { newName: "", theme: { ... } }

  Class changes:
    bg-surface -> bg-aolda-base
    text-surface -> text-aolda-base
    hover:bg-surface/50 -> hover:bg-aolda-base/50
`);
}
...
function printExportMap(): void {
  ...
  console.log(
    JSON.stringify(
      {
        meta: {
          description: "Aolda token migration map",
...
async function main(): Promise<void> {
  ...
  if (!hasPendingMigrations(renameMap)) {
    ...
    console.log('  surface: { newName: "aolda-base", theme: { ... } }');
    return;
  }

  console.log("Aolda Token Migration\n");
