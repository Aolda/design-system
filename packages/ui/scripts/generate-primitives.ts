...
// Exact export subpaths to exclude from re-export.
...
// Export subpath prefixes to exclude recursively.
...
function main() {
  ...
  const subpaths = Object.keys(exports)
    .filter((key) => key.startsWith("./") && !isExcludedExport(key))
    .sort();
...
function isExcludedExport(key: string): boolean {
  return (
...
function generateBarrelExport(subpaths: string[]) {
  const lines: string[] = [
    ...
    " * import { Popover, Slider, Tooltip } from '@aolda/ui/primitives';",
    " * ```",
    " */",
    "",
  ];
...
function generateIndividualPrimitives(subpaths: string[]) {
  for (const subpath of subpaths) {
    ...
    const content = [
      ...
      " * @example",
      " * ```tsx",
      ` * import { ${toPascalCase(primitiveName)} } from '@aolda/ui/primitives/${primitiveName}';`,
      " * ```",
      " */",
      "",
