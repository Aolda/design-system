...
export const COMPONENT_OVERRIDES: Record<string, ComponentOverride> = {
  chart: {
    ...
    sourceFile: "chart/EChart.tsx",
  },
  flow: {
    componentName: "Flow",
    propsType: "FlowProps",
    sourceFile: "flow/index.ts",
    description: "Flow - Components for visualizing workflows and data flows.",
  },
};
...
export function discoverDirs(sourceDir: string): string[] {
  ...
  return entries.filter((entry) => {
    ...
    // Most components use {name}/{name}.tsx. Compound/index-only components
    // such as chart and flow are included via source-file overrides.
    const standardMainFile = join(fullPath, `${entry}.tsx`);
    const indexFile = join(fullPath, "index.ts");
    return existsSync(standardMainFile) || existsSync(indexFile);
  });
}
 ...
 * Component names and props types are detected from index.ts exports.
 */
export async function discoverFromDir(
  ...
  for (const dirName of dirs) {
    ...
    const sourceFile = override.sourceFile || `${dirName}/${dirName}.tsx`;
    const mainFile = join(sourceDir, sourceFile);
    ...
    const componentName =
      override.componentName || detected.componentName || baseName;

    // Determine props type: detected from index.ts, then from main file, then convention
    let propsType = override.propsType || detected.propsType;
    if (!propsType) {
    ...
    // Some components (like DatePicker) don't have AOLDA_*_VARIANTS exports
    ...
    // Extract styling metadata from AOLDA_*_STYLING if present
