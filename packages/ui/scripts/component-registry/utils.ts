...
/**
 * Parse theme-aolda.css to extract semantic color names from --color-* and --text-color-* variables.
...
export function parseSemanticColorNames(): string[] {
  ...
  const themePath = join(__dirname, "../../src/styles/theme-aolda.css");
 ...
 * Returns array of Tailwind classes like ["bg-aolda-base", "text-aolda-subtle"].
 */
export function extractSemanticColors(sourceFile: string): string[] {
  try {
    ...
    // Match patterns like: text-surface, bg-primary, border-color, ring-aolda-line
    // Also handles variants like: hover:bg-primary, disabled:text-aolda-subtle
