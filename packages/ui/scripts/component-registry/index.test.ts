...
describe("extractStateClasses", () => {
  ...
  it("extracts hover state", () => {
    const classes = "bg-aolda-brand hover:bg-primary-dark";
  ...
  it("extracts focus state", () => {
    const classes = "ring-aolda-line focus:ring-aolda-hairline";
    expect(extractStateClasses(classes)).toEqual({
      focus: "focus:ring-aolda-hairline",
    });
  });

  it("extracts multiple states", () => {
    const classes =
      "bg-aolda-base hover:bg-aolda-elevated focus:ring-aolda-hairline disabled:opacity-50";
    expect(extractStateClasses(classes)).toEqual({
      hover: "hover:bg-aolda-elevated",
      focus: "focus:ring-aolda-hairline",
  ...
  it("returns empty object for no state classes", () => {
    const classes = "bg-aolda-base text-aolda-default rounded-lg";
    expect(extractStateClasses(classes)).toEqual({});
  });

  it("combines multiple classes of same state", () => {
    const classes = "hover:bg-aolda-brand hover:text-white";
    expect(extractStateClasses(classes)).toEqual({
      hover: "hover:bg-aolda-brand hover:text-white",
    });
  });
});
...
function writeTempFile(content: string): {
  ...
  const dir = mkdtempSync(join(tmpdir(), "aolda-test-"));
