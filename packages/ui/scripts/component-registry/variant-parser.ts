 ...
 *
 * Extracts AOLDA_*_VARIANTS, AOLDA_*_DEFAULT_VARIANTS, and AOLDA_*_STYLING from component files
...
/**
 * Extract AOLDA_*_BASE_STYLES from a component file.
...
export function extractBaseStylesFromFile(filePath: string): string | null {
  try {
    ...
    // Match: export const AOLDA_*_BASE_STYLES = "..." or '...' or `...`
    // Handles multi-line strings with template literals
    const baseStylesMatch = content.match(
      /export\s+const\s+AOLDA_\w+_BASE_STYLES\s*=\s*["'`]([^"'`]+)["'`]/,
    );
...
/**
 * Extract AOLDA_*_VARIANTS and AOLDA_*_DEFAULT_VARIANTS from a component file.
...
export function extractVariantsFromFile(
  ...
  try {
    ...
    // Find AOLDA_*_VARIANTS export start position
    const variantsStartMatch = content.match(
      /export\s+const\s+AOLDA_\w+_VARIANTS\s*=\s*/,
    );
    // Find AOLDA_*_DEFAULT_VARIANTS export start position
    const defaultsStartMatch = content.match(
      /export\s+const\s+AOLDA_\w+_DEFAULT_VARIANTS\s*=\s*/,
    );
...
/**
 * Extract AOLDA_*_STYLING from a component file.
...
export function extractStylingFromFile(
  ...
  try {
    ...
    // Find AOLDA_*_STYLING export start position
    const stylingStartMatch = content.match(
      /export\s+const\s+AOLDA_\w+_STYLING\s*=\s*/,
    );
