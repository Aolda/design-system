...
export const AOLDA_TABLE_VARIANTS = {
  layout: {
    auto: {
      classes: "",
...
export type AoldaTableStickyColumn = keyof typeof AOLDA_TABLE_VARIANTS.sticky;
 ...
 * `bg-aolda-elevated`) and adjust both the background and gradient fade colors.
 */
const stickyColumnClasses = (
  side: AoldaTableStickyColumn,
  /** "head" renders at z-2, "cell" at z-1 */
  element: "head" | "cell",
) => {
  const base = AOLDA_TABLE_VARIANTS.sticky[side].classes;
  ...
  if (element === "cell") {
    // Body cells always use aolda-base
    const fade =
      side === "right"
        ? "before:bg-gradient-to-r before:from-transparent before:to-aolda-base"
        : "before:bg-gradient-to-l before:from-transparent before:to-aolda-base";
    return cn(base, z, "bg-aolda-base", fadeBase, fadePosition, fade);
  }

  // Header cells: use aolda-base by default, aolda-elevated when in compact header
  // The compact header applies a data attribute we can target with :has()
  const bg = "bg-aolda-base group-data-[compact]/header:bg-aolda-elevated";
  const fade =
    side === "right"
      ? "before:bg-gradient-to-r before:from-transparent before:to-aolda-base group-data-[compact]/header:before:to-aolda-elevated"
      : "before:bg-gradient-to-l before:from-transparent before:to-aolda-base group-data-[compact]/header:before:to-aolda-elevated";

  return cn(base, z, bg, fadeBase, fadePosition, fade);
};

export const AOLDA_TABLE_DEFAULT_VARIANTS = {
  layout: "auto",
  variant: "default",
} as const;

export type AoldaTableRowVariant = keyof typeof AOLDA_TABLE_VARIANTS.variant;
export type AoldaTableLayout = keyof typeof AOLDA_TABLE_VARIANTS.layout;
...
const TableRoot = forwardRef<
  ...
  React.HTMLAttributes<HTMLTableElement> & {
     ...
     * @default "auto"
     */
    layout?: AoldaTableLayout;
  }
>(({ layout = "auto", ...props }, ref) => {
  const className = cn(
    "isolate w-full", // isolate creates a stacking context so z-0/z-1/z-2 never leak out
    AOLDA_TABLE_VARIANTS.layout[layout].classes,
    "[&_td]:border-b [&_td]:border-aolda-fill [&_tr:last-child_td]:border-b-0", // Row border
    "[&_td]:p-3", // Cell padding
    "[&_th]:border-b [&_th]:border-aolda-fill [&_th]:p-3 [&_th]:font-semibold [&_th]:text-base", // Header styles
    "[&_th]:bg-aolda-base", // Header background color
    "text-left text-aolda-default",
    props.className,
  );
...
>(({ variant = "default", sticky, ...props }, ref) => {
  ...
  const className = cn(
    "group/header",
    isCompact &&
      "[&_th]:bg-aolda-elevated [&_th]:py-2 text-xs text-aolda-strong",
...
const TableRow = forwardRef<
  ...
  React.HTMLAttributes<HTMLTableRowElement> & {
    variant?: AoldaTableRowVariant;
  }
>(({ variant = AOLDA_TABLE_DEFAULT_VARIANTS.variant, ...props }, ref) => {
  const className = cn(
    AOLDA_TABLE_VARIANTS.variant[variant].classes,
    props.className,
  );
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "m-0 bg-aolda-base p-0", // Override the stratus button styles
        "focus-visible:ring-2 focus-visible:ring-aolda-brand", // Consistent keyboard focus styling
      )}
    >
      <span className="h-5 w-[2px] rounded bg-aolda-hairline" />
    </button>
  );
});
