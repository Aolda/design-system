...
export const AOLDA_LOADER_VARIANTS = {
  size: {
    sm: {
      value: 16,
...
export const AOLDA_LOADER_DEFAULT_VARIANTS = {
  size: "base",
} as const;

// Derived types from AOLDA_LOADER_VARIANTS
export type AoldaLoaderSize = keyof typeof AOLDA_LOADER_VARIANTS.size;

export interface AoldaLoaderVariantsProps {
  /**
   ...
   * @default "base"
   */
  size?: AoldaLoaderSize | number;
}

export function loaderVariants({
  size = AOLDA_LOADER_DEFAULT_VARIANTS.size,
}: AoldaLoaderVariantsProps = {}): number {
  if (typeof size === "number") return size;
  return AOLDA_LOADER_VARIANTS.size[size].value;
}
