...
import { AOLDA_INPUT_VARIANTS, type AoldaInputSize } from "../input/input";
...
export const AOLDA_SELECT_VARIANTS = {
  size: AOLDA_INPUT_VARIANTS.size,
} as const;

export const AOLDA_SELECT_DEFAULT_VARIANTS = {
  size: "base",
} as const;

/**
 * Select component styling metadata for registry consumers.
 * Extracted from select.tsx implementation (source of truth)
 */
export const AOLDA_SELECT_STYLING = {
  trigger: {
    height: 36, // h-9
    paddingX: 12, // px-3
    borderRadius: 8, // rounded-lg
    background: "bg-aolda-elevated",
...
// Derived types from AOLDA_SELECT_VARIANTS
export type AoldaSelectSize = keyof typeof AOLDA_SELECT_VARIANTS.size;

export interface AoldaSelectVariantsProps {
  /**
   ...
   * @default "base"
   */
  size?: AoldaSelectSize;
}

export function selectVariants({
  size = AOLDA_SELECT_DEFAULT_VARIANTS.size,
}: AoldaSelectVariantsProps = {}) {
  return cn(
    buttonVariants({ size }),
    "justify-between font-normal",
    "focus:opacity-100 focus:ring-aolda-focus/50 focus-visible:ring-inset *:in-focus:opacity-100",
  );
}
...
> = {
  xs: { iconSize: 12, className: "text-aolda-subtle" },
  sm: { iconSize: 14, className: "text-aolda-subtle" },
  base: { iconSize: 16, className: "text-aolda-subtle" },
  lg: { iconSize: 18, className: "text-aolda-subtle" },
};
 ...
 * Wraps Base UI Select with Aolda styling and optional Field integration.
 *
 * @example
 * ```tsx
...
export function Select<T, Multiple extends boolean | undefined = false>({
  ...
  if (process.env.NODE_ENV !== "production" && hideLabel !== undefined) {
    console.warn(
      "[Aolda Select]: `hideLabel` is deprecated. For hidden labels, use `aria-label` instead of `label` + `hideLabel={true}`.\n" +
        "  Migration:\n" +
  ...
  const valueChildrenFn = renderValue
    ? (value: unknown) => {
        if (value == null) {
          ...
          return <span className="text-aolda-placeholder">{placeholder}</span>;
        }
  ...
  // Exclude Aolda-extended `items` from Base UI spread — we pass `normalizedItems` instead
      ...
      >
        {loading ? (
          ...
          <SelectBase.Value
            ...
            className="min-w-0 truncate data-[placeholder]:text-aolda-placeholder"
          >
            {valueChildrenFn}
          </SelectBase.Value>
        )}
      ...
      <SelectBase.Portal container={container}>
        <SelectBase.Positioner>
          <SelectBase.Popup
            className={cn(
              "flex flex-col",
              "max-h-[var(--available-height)] bg-aolda-base text-aolda-default",
              "rounded-lg shadow-lg ring ring-aolda-line",
  ...
  return (
    <div className="grid gap-2">
      ...
      {selectControl}
      {normalizedError ? (
        <span className="text-sm text-aolda-danger">
          {normalizedError.message}
        </span>
      ) : (
        description && (
          <span className="text-sm leading-snug text-aolda-subtle">
            {description}
          </span>
        )
      )}
  ...
  return (
    <SelectBase.Item
      ...
      className={cn(
        ...
        "focus-visible:z-50 focus-visible:ring-2 focus-visible:ring-aolda-brand focus-visible:ring-inset",
        "data-highlighted:bg-aolda-tint",
...
const GroupLabel = forwardRef<HTMLDivElement, GroupLabelProps>(
  ({ children, className }, ref) => (
    <SelectBase.GroupLabel
      ref={ref}
      className={cn(
        "px-3.5 py-1.5 text-sm font-semibold text-aolda-subtle",
        className,
      )}
    >
      {children}
