...
export const AOLDA_COMBOBOX_VARIANTS = {
...
export const AOLDA_COMBOBOX_DEFAULT_VARIANTS = {
  size: "base",
  inputSide: "right",
} as const;

// Context to pass size down to sub-components
const ComboboxSizeContext = createContext<AoldaInputSize>("base");

// Derived types from AOLDA_COMBOBOX_VARIANTS
export type AoldaComboboxSize = keyof typeof AOLDA_COMBOBOX_VARIANTS.size;
export type AoldaComboboxInputSide =
  keyof typeof AOLDA_COMBOBOX_VARIANTS.inputSide;

export interface AoldaComboboxVariantsProps {
  /**
   ...
   * @default "right"
   */
  inputSide?: AoldaComboboxInputSide;
}

export function comboboxVariants({
  inputSide = AOLDA_COMBOBOX_DEFAULT_VARIANTS.inputSide,
}: AoldaComboboxVariantsProps = {}) {
  return cn(AOLDA_COMBOBOX_VARIANTS.inputSide[inputSide].classes);
}

// Legacy type alias for backwards compatibility
export type ComboboxInputSide = AoldaComboboxInputSide;
export type ComboboxSize = AoldaComboboxSize;
 ...
 * ```
 */
export interface ComboboxProps extends AoldaComboboxVariantsProps {
      ...
      >
        <ComboboxBase.Popup
          className={cn(
            ...
            "bg-aolda-base text-aolda-default", // background
...
function TriggerValue({
  ...
  return (
    ...
    >
      ...
      <ComboboxBase.Icon
        className={cn(
          "absolute top-1/2 -translate-y-1/2 flex items-center text-aolda-subtle",
  ...
  return (
    ...
    >
      ...
      <ComboboxBase.Trigger
        ...
        className={cn(
          "absolute top-1/2 -translate-y-1/2 flex items-center justify-center cursor-pointer text-aolda-subtle",
...
function Item({ children, ...props }: ComboboxBase.Item.Props) {
  return (
    <ComboboxBase.Item
      {...props}
      className="group mx-1.5 grid cursor-pointer grid-cols-[1fr_16px] gap-2 rounded px-2 py-1.5 text-base data-highlighted:bg-aolda-tint"
    >
...
function Empty(props: ComboboxBase.Empty.Props) {
  return (
    <ComboboxBase.Empty
      {...props}
      className={cn(
        "mx-1.5 shrink-0 px-4 py-2 text-[0.925rem] leading-4 text-aolda-subtle empty:m-0 empty:p-0",
      )}
...
function GroupLabel(props: ComboboxBase.GroupLabel.Props) {
  return (
    <ComboboxBase.GroupLabel
      {...props}
      className={cn(
        "mx-1.5 px-2 py-1.5 text-sm text-aolda-strong",
        props.className,
      )}
    />
  );
...
function Group(props: ComboboxBase.Group.Props) {
  return (
    <ComboboxBase.Group
      {...props}
      className="border-t border-aolda-hairline mt-2 pt-2 first:border-t-0 first:mt-0 first:pt-0"
    />
  );
}
  ...
  return (
    <ComboboxBase.Chip
      ...
      className={cn(
        ...
        "rounded-sm ring-1 ring-aolda-hairline", // Border
        "bg-aolda-overlay", // Background
        "text-sm", // Typography
      )}
    >
      {props.children}
      <ComboboxBase.ChipRemove
        ...
        className={cn(
          "cursor-pointer rounded-md p-1 hover:bg-aolda-fill-hover",
          "bg-transparent flex",
        )}
      >
        <XIcon size={10} />
...
const sizeToMinHeight: Record<AoldaComboboxSize, string> = {
  xs: "min-h-5",
  sm: "min-h-6.5",
  base: "min-h-9",
  lg: "min-h-10",
