...
export const AOLDA_PAGINATION_VARIANTS = {
  controls: {
    full: {
      classes: "",
      description:
...
export type AoldaPaginationControls =
  keyof typeof AOLDA_PAGINATION_VARIANTS.controls;

export const AOLDA_PAGINATION_DEFAULT_VARIANTS = {
  controls: "full",
} as const;

export interface AoldaPaginationVariantsProps {
  controls?: AoldaPaginationControls;
}

export function paginationVariants({
  controls = AOLDA_PAGINATION_DEFAULT_VARIANTS.controls,
}: AoldaPaginationVariantsProps = {}) {
  return cn(
    "flex items-center justify-between gap-2",
    AOLDA_PAGINATION_VARIANTS.controls[controls].classes,
  );
}
...
function PaginationInfo({ children, className }: PaginationInfoProps) {
  ...
  return (
    <div
      ...
      className={cn("text-sm text-aolda-strong", className)}
    >
      {content}
    </div>
  );
...
function PaginationPageSize({
  ...
  return (
    ...
    >
      {label && <span className="text-sm text-aolda-strong">{label}</span>}
      <Select
...
export interface PaginationControlsProps extends AoldaPaginationVariantsProps {
  /**
          ...
          {controls === "full" &&
            (pageSelector === "dropdown" ? (
              <Select
                ...
                className="rounded-none ring-aolda-hairline"
                value={page}
                onValueChange={(value) => {
...
function PaginationSeparator({ className }: PaginationSeparatorProps) {
  return (
    <div
      ...
      className={cn("mx-2 h-6 border-l border-aolda-hairline", className)}
    />
  );
}
...
function PaginationRoot(props: PaginationProps) {
  ...
  const controls =
    "controls" in props
      ? (props.controls ?? AOLDA_PAGINATION_DEFAULT_VARIANTS.controls)
      : AOLDA_PAGINATION_DEFAULT_VARIANTS.controls;
    ...
    <PaginationContext.Provider value={contextValue}>
      ...
      >
        <div
          ...
          aria-atomic="true"
          data-slot="pagination-info"
          className="grow text-sm text-aolda-strong"
        >
          {getPaginationText()}
        </div>
