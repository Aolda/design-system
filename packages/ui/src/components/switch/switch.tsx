...
export const AOLDA_SWITCH_VARIANTS = {
  size: {
    sm: {
      classes: "h-5.5 w-8.5",
...
export const AOLDA_SWITCH_DEFAULT_VARIANTS = {
  size: "base",
  variant: "default",
} as const;

// Derived types from AOLDA_SWITCH_VARIANTS
export type AoldaSwitchSize = keyof typeof AOLDA_SWITCH_VARIANTS.size;
export type AoldaSwitchVariant = keyof typeof AOLDA_SWITCH_VARIANTS.variant;

export interface AoldaSwitchVariantsProps {
  /**
   * Switch size.
   ...
   * @default "default"
   */
  variant?: AoldaSwitchVariant;
}

export function switchVariants({
  size = AOLDA_SWITCH_DEFAULT_VARIANTS.size,
  variant = AOLDA_SWITCH_DEFAULT_VARIANTS.variant,
}: AoldaSwitchVariantsProps = {}) {
  // Fallback to defaults if invalid size/variant passed
  const sizeConfig =
    AOLDA_SWITCH_VARIANTS.size[size] ?? AOLDA_SWITCH_VARIANTS.size.base;
  const variantConfig =
    AOLDA_SWITCH_VARIANTS.variant[variant] ??
    AOLDA_SWITCH_VARIANTS.variant.default;
  return cn(sizeConfig.classes, variantConfig.classes);
}

// Legacy type aliases for backwards compatibility
export type SwitchSize = AoldaSwitchSize;
export type SwitchVariant = AoldaSwitchVariant;
      ...
      <BaseSwitch.Root
        ...
        render={(rootProps, state) => {
          ...
          const trackColors = isNeutral
            ? state.checked
              ? "bg-aolda-contrast ring-aolda-contrast"
              : "bg-aolda-fill ring-aolda-hairline"
            : state.checked
              ? "bg-aolda-brand ring-aolda-brand"
              : "bg-aolda-fill ring-aolda-line";
          ...
          const trackClassName = cn(
            ...
            "focus:outline-none focus-visible:ring-2 focus-visible:ring-aolda-brand",
          ...
          const thumbClassName = cn(
            "absolute top-0 bottom-0 shadow-[0_0_1px_0.5px_var(--color-aolda-shadow-edge),0_1px_2px_var(--color-aolda-shadow-drop)]",
            s.thumb,
            squircleRadius,
            thumbColors,
