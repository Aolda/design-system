...
export const AOLDA_TABLE_OF_CONTENTS_VARIANTS = {
  state: {
    default: {
      classes:
        "text-aolda-subtle hover:border-aolda-line hover:text-aolda-default hover:font-medium",
      description: "Inactive section link",
    },
    active: {
      classes: "border-aolda-brand font-medium text-aolda-default",
      description: "Currently visible / active section",
    },
  },
} as const;

export const AOLDA_TABLE_OF_CONTENTS_DEFAULT_VARIANTS = {
  state: "default",
} as const;

export type AoldaTableOfContentsState =
  keyof typeof AOLDA_TABLE_OF_CONTENTS_VARIANTS.state;
...
>(({ className, ...props }, ref) => (
  <p
    ref={ref}
    className={cn(
      "mb-3 text-xs font-semibold tracking-wide text-aolda-subtle uppercase",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ul
    ref={ref}
    className={cn(
      "flex flex-col gap-2 border-l-2 border-aolda-hairline",
      className,
    )}
    {...props}
  />
...
>(({ active = false, className, children, render, ...props }, ref) => {
  const stateClasses = active
    ? AOLDA_TABLE_OF_CONTENTS_VARIANTS.state.active.classes
    : AOLDA_TABLE_OF_CONTENTS_VARIANTS.state.default.classes;
...
const NESTED_UL_CLASSES =
  "flex flex-col gap-2 border-l-2 border-aolda-hairline [&>li>a]:pl-7 [&>li>button]:pl-7";
...
>(({ label, href, active = false, className, children, ...props }, ref) => {
  ...
  return (
    <li
      ...
      {...props}
    >
      <p className="py-0.5 pl-4 text-sm leading-5 font-medium text-aolda-subtle">
        {label}
      </p>
      <ul className={cn(NESTED_UL_CLASSES)}>{children}</ul>
    </li>
