...
export const AOLDA_DATE_RANGE_PICKER_VARIANTS = {
  size: {
    sm: {
      classes: "p-3 gap-2",
      cellHeight: "h-[22px]",
  ...
  variant: {
    default: {
      classes: "bg-aolda-overlay",
      description: "Default calendar appearance",
    },
    subtle: {
      classes: "bg-aolda-base",
      description: "Subtle calendar with minimal background",
    },
  },
} as const;

export const AOLDA_DATE_RANGE_PICKER_DEFAULT_VARIANTS = {
  size: "base",
  variant: "default",
} as const;

// Derived types from AOLDA_DATE_RANGE_PICKER_VARIANTS
export type AoldaDateRangePickerSize =
  keyof typeof AOLDA_DATE_RANGE_PICKER_VARIANTS.size;
export type AoldaDateRangePickerVariant =
  keyof typeof AOLDA_DATE_RANGE_PICKER_VARIANTS.variant;

export interface AoldaDateRangePickerVariantsProps {
  /**
   * Calendar size.
   ...
   * @default "default"
   */
  variant?: AoldaDateRangePickerVariant;
}

export function dateRangePickerVariants({
  size = AOLDA_DATE_RANGE_PICKER_DEFAULT_VARIANTS.size,
  variant = AOLDA_DATE_RANGE_PICKER_DEFAULT_VARIANTS.variant,
}: AoldaDateRangePickerVariantsProps = {}) {
  return cn(
    // Base styles
    "flex w-fit flex-col rounded-xl select-none",
    // Apply variant and size styles
    AOLDA_DATE_RANGE_PICKER_VARIANTS.variant[variant].classes,
    AOLDA_DATE_RANGE_PICKER_VARIANTS.size[size].classes,
  );
}

// Helper to get size config
function getSizeConfig(size: AoldaDateRangePickerSize) {
  return AOLDA_DATE_RANGE_PICKER_VARIANTS.size[size];
}
...
export interface DateRangePickerProps
  extends AoldaDateRangePickerVariantsProps {
      ...
      <div className="flex gap-4">
        <div className={cn("relative", sizeConfig.calendarWidth)}>
          <button
            ...
            className="absolute top-0 left-0 cursor-pointer rounded bg-aolda-interact/85 p-1.5 hover:bg-aolda-interact"
        ...
        <div className={cn("relative", sizeConfig.calendarWidth)}>
          <button
            ...
            className="absolute top-0 right-0 cursor-pointer rounded bg-aolda-interact/85 p-1.5 hover:bg-aolda-interact"
            onClick={() => adjustMonth(1)}
          >
...
function DateRangeDayCell({
  ...
  const getBackgroundColor = useCallback(() => {
    switch (mode) {
      ...
      case DateRangeCellMode.SELECTED_START_NODE:
        return "!bg-aolda-contrast rounded-tl-[5px] rounded-bl-[5px]";
      case DateRangeCellMode.SELECTED_END_NODE:
        return "!bg-aolda-contrast rounded-tr-[5px] rounded-br-[5px]";
      case DateRangeCellMode.SELECTED:
        return "bg-aolda-interact";
      case DateRangeCellMode.SELECTED_OUT_OF_RANGE:
        return "bg-aolda-fill";
    }
  }, [mode]);
  ...
  const getTextColor = useCallback(() => {
    switch (mode) {
      case DateRangeCellMode.OUT_OF_RANGE:
      case DateRangeCellMode.SELECTED_OUT_OF_RANGE:
        return "!text-aolda-strong";
      case DateRangeCellMode.SELECTED_START_NODE:
      case DateRangeCellMode.SELECTED_END_NODE:
        return "!text-aolda-inverse";
      default:
        return "text-aolda-default";
    }
  }, [mode]);
  ...
  return (
    <button
      ...
      className={cn(
        ...
        sizeConfig.textSize,
        "cursor-pointer text-center text-aolda-default transition-all duration-[50]",
    ...
    <div>
      <div className="mb-3 text-center">
        <input
          ...
          className={cn(
            "w-full rounded-md border-none bg-transparent py-1.5 text-center font-semibold text-aolda-default transition-all duration-200 focus:outline-none focus:ring-aolda-focus/50 focus:ring-[1.5px]",
            sizeConfig.textSize,
          )}
          onBlur={(e) => {
      ...
      <div className="mt-2 grid grid-cols-7 gap-1">
        {DAYS_OF_WEEK.map((day) => (
          <div
            key={day}
            className={cn(
              "h-[22px] text-center text-aolda-subtle",
              sizeConfig.cellWidth,
              sizeConfig.textSize,
            )}
          >
...
function DateRangeFooter({
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-2 text-aolda-strong",
        sizeConfig.textSize,
      )}
    >
      ...
      <button
        type="button"
        onClick={reset}
        className="cursor-pointer font-semibold text-aolda-default underline underline-offset-2"
      >
        Reset Dates
      </button>
    </div>
