...
export const AOLDA_CHECKBOX_VARIANTS = {
  variant: {
    default: {
      classes:
        "[&:focus-within>span]:ring-aolda-focus [&:hover>span]:ring-aolda-hairline",
      description: "Default checkbox appearance",
    },
    error: {
      classes: "[&>span]:ring-aolda-danger",
      description: "Error state for validation failures",
    },
  },
} as const;

export const AOLDA_CHECKBOX_DEFAULT_VARIANTS = {
  variant: "default",
} as const;

// Derived types from AOLDA_CHECKBOX_VARIANTS
export type AoldaCheckboxVariant = keyof typeof AOLDA_CHECKBOX_VARIANTS.variant;

export interface AoldaCheckboxVariantsProps {
  /**
   * Visual variant.
   ...
   * @default "default"
   */
  variant?: AoldaCheckboxVariant;
}

export function checkboxVariants({
  variant = AOLDA_CHECKBOX_DEFAULT_VARIANTS.variant,
}: AoldaCheckboxVariantsProps = {}) {
  return cn(AOLDA_CHECKBOX_VARIANTS.variant[variant].classes);
}

// Legacy type alias for backwards compatibility
export type CheckboxVariant = AoldaCheckboxVariant;
  ...
  (
    ...
    if (process.env.NODE_ENV !== "production") {
      ...
      if (!hasLabel && !hasAriaLabel && !hasAriaLabelledBy) {
        console.warn(
          "[Aolda Checkbox]: Checkbox must have an accessible name. Provide either:\n" +
    ...
    const checkboxControl = (
      <BaseCheckbox.Root
        ...
        className={cn(
          "relative flex h-4 w-4 items-center justify-center rounded-sm border-0 bg-aolda-base ring focus:outline-none after:absolute after:-inset-x-3 after:-inset-y-2",
          variant === "error" ? "ring-aolda-danger" : "ring-aolda-hairline",
          !disabled &&
            "hover:ring-aolda-hairline focus:ring-aolda-focus focus:ring-2 focus-visible:ring-2 focus-visible:ring-aolda-brand",
          "data-[checked]:bg-aolda-brand data-[checked]:ring-aolda-brand data-[indeterminate]:bg-aolda-brand data-[indeterminate]:ring-aolda-brand",
    ...
    return (
      ...
      >
        <BaseCheckbox.Root
          ...
          onCheckedChange={onCheckedChange}
          className={cn(
            "peer relative flex h-4 w-4 items-center justify-center rounded-sm border-0 bg-aolda-base ring after:absolute after:-inset-x-3 after:-inset-y-2",
            variant === "error" ? "ring-aolda-danger" : "ring-aolda-hairline",
            !disabled &&
              "group-hover:ring-aolda-hairline hover:ring-aolda-hairline focus:ring-aolda-focus focus:ring-2 focus-visible:ring-2 focus-visible:ring-aolda-brand",
            "data-[checked]:bg-aolda-brand data-[checked]:ring-aolda-brand data-[indeterminate]:bg-aolda-brand data-[indeterminate]:ring-aolda-brand",
          )}
        >
          <BaseCheckbox.Indicator
