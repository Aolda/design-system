...
export const AOLDA_LABEL_VARIANTS = {
  // Label currently has no variant options but structure is ready for future additions
} as const;

export const AOLDA_LABEL_DEFAULT_VARIANTS = {} as const;

// Derived types from AOLDA_LABEL_VARIANTS
export interface AoldaLabelVariantsProps {}

export function labelVariants(_props: AoldaLabelVariantsProps = {}) {
  return cn(
    ...
    "m-0 text-base font-medium text-aolda-default",
  );
}
 ...
 * ```
 */
export interface LabelProps extends AoldaLabelVariantsProps {
...
export function Label({
  ...
  const content = (
    <>
      {children}
      {showOptional && (
        <span className="font-normal text-aolda-strong">(optional)</span>
      )}
      {tooltip && (
        <Tooltip
          content={tooltip}
