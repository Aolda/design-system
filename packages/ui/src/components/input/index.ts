...
// `@aolda/ui/components/input` continues to resolve InputGroup.
export {
  InputGroup,
  AOLDA_INPUT_GROUP_VARIANTS,
  AOLDA_INPUT_GROUP_DEFAULT_VARIANTS,
} from "../input-group";

// Backward-compatible type aliases — the old `input-group.tsx` exported these
// names. External consumers importing from `@aolda/ui/components/input`
 ...
 * major version.
 */
export type AoldaInputGroupFocusMode = "container" | "individual";

/**
 * @deprecated `focusMode` is no longer a public prop — it is auto-detected by
 * `InputGroup` based on its children. Use `InputGroupRootProps` from
 * `@aolda/ui` instead. This type will be removed in a future major
 * version.
 */
export interface AoldaInputGroupVariantsProps {
  focusMode?: AoldaInputGroupFocusMode;
}
