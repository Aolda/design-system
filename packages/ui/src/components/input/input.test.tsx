...
describe("Input", () => {
  ...
  it("warns in dev when no accessible name is provided", () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining("[Aolda Input]"),
    );
    warnSpy.mockRestore();
  });
  ...
  it("returns base classes with default arguments", () => {
    ...
    expect(classes).toContain("bg-aolda-control");
    expect(classes).toContain("text-aolda-default");
  });

  it("applies size classes from AOLDA_INPUT_VARIANTS", () => {
    const classes = inputVariants({ size: "lg" });
    expect(classes).toContain("h-10");
    expect(classes).toContain("px-4");
  });

  it("applies variant classes from AOLDA_INPUT_VARIANTS", () => {
    const classes = inputVariants({ variant: "error" });
    expect(classes).toContain("ring-aolda-danger");
  });
  ...
  it("applies focusIndicator class when true", () => {
    ...
    expect(classes).toContain("focus:ring-aolda-focus/50");
  });

  // Variants export
  it("exports AOLDA_INPUT_VARIANTS with size and variant axes", () => {
    expect(AOLDA_INPUT_VARIANTS.size.xs).toBeDefined();
    expect(AOLDA_INPUT_VARIANTS.size.sm).toBeDefined();
    expect(AOLDA_INPUT_VARIANTS.size.base).toBeDefined();
    expect(AOLDA_INPUT_VARIANTS.size.lg).toBeDefined();
    expect(AOLDA_INPUT_VARIANTS.variant.default).toBeDefined();
    expect(AOLDA_INPUT_VARIANTS.variant.error).toBeDefined();
  });

  it("exports AOLDA_INPUT_DEFAULT_VARIANTS with correct defaults", () => {
    expect(AOLDA_INPUT_DEFAULT_VARIANTS.size).toBe("base");
    expect(AOLDA_INPUT_DEFAULT_VARIANTS.variant).toBe("default");
  });
});
