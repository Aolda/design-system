...
export const AOLDA_INPUT_VARIANTS = {
  size: {
    xs: {
  ...
  variant: {
    default: {
      classes: "focus:ring-aolda-focus/50 focus:ring-[1.5px]",
      description: "Default input appearance",
    },
    error: {
      classes:
        "!ring-aolda-danger focus:ring-aolda-danger/50 focus:ring-[1.5px]",
      description: "Error state for validation failures",
    },
  },
} as const;

export const AOLDA_INPUT_DEFAULT_VARIANTS = {
  size: "base",
  variant: "default",
} as const;

export const AOLDA_INPUT_STYLING = {
  dimensions: {
...
// Derived types from AOLDA_INPUT_VARIANTS
export type AoldaInputSize = keyof typeof AOLDA_INPUT_VARIANTS.size;
export type AoldaInputVariant = keyof typeof AOLDA_INPUT_VARIANTS.variant;

export interface AoldaInputVariantsProps {
  /**
   * Input size.
...
export function inputVariants({
  variant = AOLDA_INPUT_DEFAULT_VARIANTS.variant,
  size = AOLDA_INPUT_DEFAULT_VARIANTS.size,
  parentFocusIndicator = false,
  focusIndicator = false,
}: AoldaInputVariantsProps = {}) {
  return cn(
    // Base styles
    "border-0 bg-aolda-control text-aolda-default ring ring-aolda-line outline-none focus:outline-none",
    // Disabled state and placeholder styles (using vanilla CSS class for Chrome compatibility)
    "aolda-input-placeholder disabled:text-aolda-subtle",
    // Apply size styles from AOLDA_INPUT_VARIANTS
    AOLDA_INPUT_VARIANTS.size[size].classes,
    // Apply variant styles from AOLDA_INPUT_VARIANTS
    AOLDA_INPUT_VARIANTS.variant[variant].classes,
    // Focus state handling
    parentFocusIndicator &&
      (variant === "error"
        ? "focus-within:ring-aolda-danger/50 focus-within:ring-[1.5px]"
        : "focus-within:ring-aolda-focus/50 focus-within:ring-[1.5px]"),
    focusIndicator &&
      (variant === "error"
        ? "focus:ring-aolda-danger/50 focus:ring-[1.5px]"
        : "focus:ring-aolda-focus/50 focus:ring-[1.5px]"),
  );
}
...
export const Input = forwardRef<HTMLInputElement, InputProps>((props, ref) => {
  ...
  if (process.env.NODE_ENV !== "production" && variantProp === "error") {
    console.warn(
      '[Aolda Input]: variant="error" is deprecated. ' +
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    if (!hasLabel && !hasAriaLabel && !hasAriaLabelledBy) {
      console.warn(
        "[Aolda Input]: Input must have an accessible name. Provide either:\n" +
 ...
 * />
 */
export type InputProps = Pick<AoldaInputVariantsProps, "size" | "variant"> &
  BaseInputProps & {
