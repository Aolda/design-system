...
export const AOLDA_POPOVER_VARIANTS = {
  side: {
    top: {
      classes: "",
...
export const AOLDA_POPOVER_DEFAULT_VARIANTS = {
  side: "bottom",
} as const;

// Derived types from AOLDA_POPOVER_VARIANTS
export type AoldaPopoverSide = keyof typeof AOLDA_POPOVER_VARIANTS.side;

export interface AoldaPopoverVariantsProps {
  /**
 ...
 * ```
 */
export type PopoverContentProps = AoldaPopoverVariantsProps & {
  /**
...
function PopoverContent({
  children,
  side = AOLDA_POPOVER_DEFAULT_VARIANTS.side,
  align = "center",
  sideOffset = 8,
  alignOffset = 0,
  ...
  return (
    <PopoverBase.Portal container={container}>
      ...
      >
        <PopoverBase.Popup
          className={cn(
            "flex origin-(--transform-origin) flex-col rounded-lg bg-aolda-base px-4 py-3 text-sm text-aolda-default",
...
function PopoverDescription({ className, ...props }: PopoverDescriptionProps) {
  return (
    <PopoverBase.Description
      className={cn("m-0 text-base leading-6 text-aolda-subtle", className)}
      {...props}
    />
  );
}
