...
export const AOLDA_LINK_VARIANTS = {
  variant: {
    inline: {
      classes:
        // text-aolda-link provides defensive color that won't be overridden by global `a` styles
        "text-aolda-link underline underline-offset-[0.15em] decoration-[0.0625em] link-current transition-colors",
    ...
    plain: {
      classes:
        // text-aolda-link provides defensive color that won't be overridden by global `a` styles
        "text-aolda-link hover:text-aolda-link/70 transition-colors",
      description: "Link without underline decoration",
    },
  },
} as const;

export const AOLDA_LINK_DEFAULT_VARIANTS = {
  variant: "inline",
} as const;

export type AoldaLinkVariant = keyof typeof AOLDA_LINK_VARIANTS.variant;

export interface AoldaLinkVariantsProps {
  /**
   ...
   * @default "inline"
   */
  variant?: AoldaLinkVariant;
}

export function linkVariants({
  variant = AOLDA_LINK_DEFAULT_VARIANTS.variant,
}: AoldaLinkVariantsProps = {}) {
  return cn(AOLDA_LINK_VARIANTS.variant[variant].classes);
}
