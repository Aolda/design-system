...
export const AOLDA_CLIPBOARD_TEXT_VARIANTS = {
  size: {
    sm: {
      classes: "text-xs",
...
export const AOLDA_CLIPBOARD_TEXT_DEFAULT_VARIANTS = {
  size: "lg",
} as const;
...
// Derived types from AOLDA_CLIPBOARD_TEXT_VARIANTS
export type AoldaClipboardTextSize =
  keyof typeof AOLDA_CLIPBOARD_TEXT_VARIANTS.size;

export interface AoldaClipboardTextVariantsProps {
  /**
   ...
   * @default "lg"
   */
  size?: AoldaClipboardTextSize;
}

export function clipboardTextVariants({
  size = AOLDA_CLIPBOARD_TEXT_DEFAULT_VARIANTS.size,
}: AoldaClipboardTextVariantsProps = {}) {
  return cn(
    // Base styles
    "flex items-center overflow-hidden bg-aolda-base px-0 font-mono",
    // Apply size styles from AOLDA_CLIPBOARD_TEXT_VARIANTS
    AOLDA_CLIPBOARD_TEXT_VARIANTS.size[size].classes,
  );
}

// Legacy type alias for backwards compatibility
export type ClipboardTextSize = AoldaClipboardTextSize;
 ...
 * <ClipboardText text="npm install @aolda/ui" size="sm" />
 * ```
 */
export interface ClipboardTextProps extends AoldaClipboardTextVariantsProps {
      ...
      {toasts.map((toast) => (
        <Toast.Positioner key={toast.id} toast={toast} className="absolute">
          <Toast.Root
            toast={toast}
            className={cn(
              "flex origin-[var(--transform-origin)] flex-col rounded-md bg-aolda-base px-3 py-1.5 text-xs text-aolda-default font-sans",
              "shadow-lg shadow-aolda-tip-shadow outline outline-aolda-fill",
            )}
          >
            <Toast.Description />
          </Toast.Root>
...
export const ClipboardText = forwardRef<HTMLDivElement, ClipboardTextProps>(
  (
    ...
    const sizeConfig = AOLDA_CLIPBOARD_TEXT_VARIANTS.size[size];
    ...
    const copyButton = (
      <Button
        ...
        variant="ghost"
        className={cn(
          "rounded-l-none rounded-r-[inherit] border-l! border-aolda-line! px-3 relative isolate overflow-hidden transition-all duration-200",
          "focus:ring-inset focus:ring-aolda-focus/50",
          "focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-aolda-brand",
        )}
            ...
            >
              ...
              <Tooltip.Portal>
                <Tooltip.Positioner side={tooltipSide} sideOffset={8}>
                  <Tooltip.Popup
                    className={cn(
                      "flex origin-[var(--transform-origin)] flex-col rounded-md bg-aolda-base px-3 py-1.5 text-xs text-aolda-default",
                      "shadow-lg shadow-aolda-tip-shadow outline outline-aolda-fill",
                    )}
                  >
                    {tooltipText}
