...
export const AOLDA_RADIO_VARIANTS = {
  variant: {
    default: {
      classes: "ring-aolda-hairline",
      description: "Default radio appearance",
    },
    error: {
      classes: "ring-aolda-danger",
  ...
  appearance: {
    ...
    card: {
      classes:
        "rounded-lg border border-aolda-hairline bg-aolda-base p-3 transition-colors hover:bg-aolda-tint has-[[data-checked]]:border-aolda-interact has-[[data-checked]]:bg-aolda-tint",
      description:
...
export const AOLDA_RADIO_DEFAULT_VARIANTS = {
  variant: "default",
  appearance: "default",
} as const;

// Derived types from AOLDA_RADIO_VARIANTS
export type AoldaRadioVariant = keyof typeof AOLDA_RADIO_VARIANTS.variant;
export type AoldaRadioAppearance = keyof typeof AOLDA_RADIO_VARIANTS.appearance;

export interface AoldaRadioVariantsProps {
  /**
   * Visual variant.
   ...
   * @default "default"
   */
  appearance?: AoldaRadioAppearance;
}

export function radioVariants({
  variant = AOLDA_RADIO_DEFAULT_VARIANTS.variant,
  appearance = AOLDA_RADIO_DEFAULT_VARIANTS.appearance,
}: AoldaRadioVariantsProps = {}) {
  return cn(
    AOLDA_RADIO_VARIANTS.variant[variant].classes,
    AOLDA_RADIO_VARIANTS.appearance[appearance].classes,
  );
}

// Legacy type alias for backwards compatibility
export type RadioVariant = AoldaRadioVariant;
  ...
  (
    ...
    if (isCard) {
      ...
      return (
        <label
          className={cn(
            "m-0 group relative flex items-start gap-3 rounded-lg border border-aolda-hairline bg-aolda-base p-3 transition-colors has-[[data-checked]]:border-aolda-interact has-[[data-checked]]:bg-aolda-tint",
            controlAtStart && "flex-row-reverse",
            variant === "error" &&
              "border-aolda-danger has-[[data-checked]]:border-aolda-danger has-[[data-checked]]:bg-aolda-base",
            disabled
              ? "cursor-not-allowed opacity-50"
              : cn(
                  "has-[[data-disabled]]:cursor-not-allowed has-[[data-disabled]]:opacity-50 cursor-pointer",
                  variant !== "error" &&
                    "hover:not-has-[[data-disabled]]:bg-aolda-tint",
                ),
            className,
          )}
        >
          <div className="flex min-w-0 flex-1 flex-col gap-0.5">
            <span className="text-base font-medium text-aolda-default">
              {label}
            </span>
            {description && (
              <span className="text-sm text-aolda-subtle">{description}</span>
            )}
          </div>
          <BaseRadio.Root
            ref={ref}
            ...
            disabled={disabled}
            className={cn(
              "relative mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full border-0 bg-aolda-base ring ring-2 focus:outline-none focus:ring-aolda-focus focus-visible:ring-2 focus-visible:ring-aolda-brand",
              variant === "error" ? "ring-aolda-danger" : "ring-aolda-line",
              !disabled &&
                variant !== "error" &&
                "group-hover:ring-aolda-hairline focus-visible:outline-offset-3",
              !disabled &&
                variant === "error" &&
                "focus-visible:outline-offset-3",
              "data-[checked]:bg-aolda-brand data-[checked]:ring-aolda-brand",
            )}
          >
            <BaseRadio.Indicator
              keepMounted
              className="flex items-center justify-center data-[unchecked]:invisible"
            >
    ...
    return (
      ...
      >
        <BaseRadio.Root
          ...
          disabled={disabled}
          className={cn(
            "relative flex h-4 w-4 items-center justify-center rounded-full border-0 bg-aolda-base ring focus:outline-none after:absolute after:-inset-x-3 after:-inset-y-2",
            variant === "error" ? "ring-aolda-danger" : "ring-aolda-line",
            !disabled &&
              variant !== "error" &&
              "group-hover:ring-aolda-hairline focus:ring-aolda-focus focus:ring-2 focus-visible:ring-2 focus-visible:ring-aolda-brand focus-visible:outline-offset-3",
            !disabled &&
              variant === "error" &&
              "focus:ring-aolda-focus focus:ring-2 focus-visible:ring-2 focus-visible:ring-aolda-brand focus-visible:outline-offset-3",
            "data-[checked]:bg-aolda-brand data-[checked]:ring-aolda-brand",
          )}
        >
          <BaseRadio.Indicator
        ...
        <span className="text-base text-aolda-default">{label}</span>
      </label>
    );
  },
);
...
function RadioLegend({ children, className }: RadioLegendProps) {
  return (
    <Fieldset.Legend
      className={cn("text-base font-medium text-aolda-default", className)}
    >
      {children}
    </Fieldset.Legend>
  );
  ...
  return (
    <RadioGroupContext.Provider value={{ controlPosition, appearance }}>
      ...
      >
        ...
        >
          {legend && (
            <Fieldset.Legend className="text-base font-medium text-aolda-default">
              {legend}
            </Fieldset.Legend>
          )}
          <div
          ...
          {error && <p className="text-sm text-aolda-danger">{error}</p>}
          {description && (
            <p className="text-sm text-aolda-subtle">{description}</p>
          )}
        </Fieldset.Root>
      </BaseRadioGroup>
