...
export const AOLDA_TOOLTIP_VARIANTS = {
  side: {
    top: {
      classes: "",
...
export const AOLDA_TOOLTIP_DEFAULT_VARIANTS = {
  side: "top",
} as const;

// Derived types from AOLDA_TOOLTIP_VARIANTS
export type AoldaTooltipSide = keyof typeof AOLDA_TOOLTIP_VARIANTS.side;

export interface AoldaTooltipVariantsProps {
  /**
   ...
   * @default "top"
   */
  side?: AoldaTooltipSide;
}

export function tooltipVariants({
  side = AOLDA_TOOLTIP_DEFAULT_VARIANTS.side,
}: AoldaTooltipVariantsProps = {}) {
  return cn(
    // Base styles
    "flex origin-[var(--transform-origin)] flex-col rounded-md bg-aolda-base px-2.5 py-1.5 text-sm text-aolda-default",
    "shadow-lg shadow-aolda-tip-shadow outline outline-1 outline-aolda-fill",
    "transition-[transform,opacity] duration-150",
    "data-[starting-style]:scale-90 data-[starting-style]:opacity-0",
    "data-[ending-style]:scale-90 data-[ending-style]:opacity-0",
    "data-[instant]:duration-0",
    // Apply side-specific styles (currently none, but extensible)
    AOLDA_TOOLTIP_VARIANTS.side[side].classes,
  );
}
