...
export const AOLDA_DIALOG_VARIANTS = {
  size: {
    base: {
      classes: "sm:min-w-96",
...
export const AOLDA_DIALOG_DEFAULT_VARIANTS = {
  size: "base",
  role: "dialog",
} as const;

export const AOLDA_DIALOG_STYLING = {
  dimensions: {
    sm: {
      width: 350,
      titleSize: 20,
...
// Derived types from AOLDA_DIALOG_VARIANTS
export type AoldaDialogSize = keyof typeof AOLDA_DIALOG_VARIANTS.size;
export type AoldaDialogRole = keyof typeof AOLDA_DIALOG_VARIANTS.role;

export interface AoldaDialogVariantsProps {
  /**
   * Dialog width.
...
const DialogRoleContext = createContext<AoldaDialogRole>("dialog");
...
export function dialogVariants({
  size = AOLDA_DIALOG_DEFAULT_VARIANTS.size,
}: AoldaDialogVariantsProps = {}) {
  return cn(
    // Base styles
    "shadow-m ring ring-aolda-line fixed top-1/2 left-1/2 w-full sm:w-auto max-w-[calc(100vw-2rem)] sm:max-w-[calc(100vw-3rem)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-xl bg-aolda-base text-aolda-default duration-150 data-ending-style:scale-90 data-ending-style:opacity-0 data-starting-style:scale-90 data-starting-style:opacity-0",
    // Apply size from AOLDA_DIALOG_VARIANTS
    AOLDA_DIALOG_VARIANTS.size[size].classes,
  );
}
 ...
 * ```
 */
export type DialogProps = AoldaDialogVariantsProps & {
...
function DialogContent({
  ...
  return (
    <BasePortal container={container}>
      <BaseBackdrop className="fixed inset-0 bg-aolda-recessed opacity-80 transition-all duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0" />
      <LayerCard
        render={<BasePopup />}
...
export type DialogRootProps = BaseDialogRootProps & {
   ...
   * @default "dialog"
   */
  role?: AoldaDialogRole;
};

function DialogRoot({
  children,
  role = AOLDA_DIALOG_DEFAULT_VARIANTS.role,
  ...props
}: DialogRootProps) {
  const BaseRoot =
