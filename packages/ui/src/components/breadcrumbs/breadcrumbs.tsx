...
export const AOLDA_BREADCRUMBS_VARIANTS = {
  size: {
    sm: {
...
export const AOLDA_BREADCRUMBS_DEFAULT_VARIANTS = {
  size: "base",
} as const;

export type AoldaBreadcrumbsSize = keyof typeof AOLDA_BREADCRUMBS_VARIANTS.size;

export interface AoldaBreadcrumbsVariantsProps {
  /**
   ...
   * @default "base"
   */
  size?: AoldaBreadcrumbsSize;
}

export function breadcrumbsVariants({
  size = AOLDA_BREADCRUMBS_DEFAULT_VARIANTS.size,
}: AoldaBreadcrumbsVariantsProps = {}) {
  return cn(
    "group mr-4 flex min-w-0 grow items-center overflow-hidden whitespace-nowrap",
    AOLDA_BREADCRUMBS_VARIANTS.size[size].classes,
  );
}
...
const Link = ({
  ...
  return (
    <LinkComponent
      to={href}
      className="flex min-w-0 max-w-full items-center gap-1 text-aolda-subtle no-underline"
    >
...
function Separator() {
  return (
    <span
      className="flex shrink-0 items-center text-aolda-inactive"
      aria-hidden="true"
    >
...
function MobileEllipsis() {
  return (
    <span className="flex shrink-0 items-center text-aolda-subtle" aria-hidden>
      ...
    </span>
  );
}
...
function Clipboard({ text }: { text: string }) {
  ...
  return (
    ...
    >
      {isCopied ? (
        <CheckIcon weight="bold" className="text-aolda-success" />
      ) : (
...
function getMobileBreadcrumbChildren(children: ReactNode[]): ReactNode[] {
  ...
  const trailingItems: ReactNode[] = [
    <MobileEllipsis key="aolda-breadcrumb-mobile-ellipsis" />,
    <Separator key="aolda-breadcrumb-mobile-separator-leading" />,
    cloneElement(parentItem, { key: "aolda-breadcrumb-mobile-parent" }),
    <Separator key="aolda-breadcrumb-mobile-separator-trailing" />,
    cloneElement(currentItem, { key: "aolda-breadcrumb-mobile-current" }),
  ];
