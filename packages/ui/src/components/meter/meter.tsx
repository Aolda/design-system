...
export const AOLDA_METER_VARIANTS = {
  // Meter currently has no variant options but structure is ready for future additions
} as const;

export const AOLDA_METER_DEFAULT_VARIANTS = {} as const;

// Derived types from AOLDA_METER_VARIANTS
export interface AoldaMeterVariantsProps {}

export function meterVariants(_props: AoldaMeterVariantsProps = {}) {
  return cn(
    // Base styles
 ...
 * ```
 */
export interface MeterProps extends RootProps, AoldaMeterVariantsProps {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-4">
        <BaseMeter.Label className="text-xs text-aolda-strong">
          {label}
        </BaseMeter.Label>
        {customValue ? (
          <span className="text-sm font-medium text-aolda-default tabular-nums">
            {customValue}
          </span>
        ) : (
          <>
            {showValue && (
              <BaseMeter.Value className="text-sm font-medium text-aolda-default tabular-nums" />
            )}
          </>
        )}
      </div>
      <BaseMeter.Track
        className={cn(
          "relative h-2 w-full overflow-hidden rounded-full bg-aolda-fill",
          trackClassName,
        )}
      >
        <BaseMeter.Indicator
          className={cn(
            "absolute inset-y-0 left-0 rounded-full bg-linear-to-r from-aolda-brand via-aolda-brand to-aolda-brand transition-[width] duration-300 ease-out",
            indicatorClassName,
          )}
        />
      </BaseMeter.Track>
