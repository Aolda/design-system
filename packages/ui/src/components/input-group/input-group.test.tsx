...
import type { AoldaInputSize } from "../input/input";
...
describe("InputGroup", () => {
  ...
  describe("size variants", () => {
    ...
    it.each(["xs", "sm", "base", "lg"] as const)(
      ...
      (size: AoldaInputSize) => {
        const labels: Record<AoldaInputSize, string> = {
          xs: "Extra Small",
          sm: "Small",
          base: "Base (default)",
          lg: "Large",
  ...
  describe("stratus backward compatibility", () => {
    it("supports legacy search bar pattern with Label", () => {
      render(
        <InputGroup className="bg-aolda-base flex-1">
          <InputGroup.Label>
            <svg data-testid="search-icon" />
          </InputGroup.Label>
  ...
  describe("focus mode auto-detection (rendered)", () => {
    ...
    it("hybrid mode gets borderless container classes (same as individual)", () => {
      ...
      expect(group.className).not.toContain("focus-within:ring-aolda-focus");
    });
    ...
    it("container mode gets shared focus ring classes", () => {
      ...
      expect(group.className).toContain("focus-within:ring-aolda-focus");
  ...
  describe("hybrid mode", () => {
    ...
    it("container zone has correct border and overflow styling", () => {
      ...
      expect(containerZone.className).toContain("border-aolda-line");
      ...
      // No focus-within ring — CSS outline in aolda-binding.css handles focus
      expect(containerZone.className).not.toContain(
        "focus-within:ring-aolda-line",
      );
    });
  });
});
