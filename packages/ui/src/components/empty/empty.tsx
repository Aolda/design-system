...
export const AOLDA_EMPTY_VARIANTS = {
  size: {
    sm: {
...
export const AOLDA_EMPTY_DEFAULT_VARIANTS = {
  size: "base",
} as const;

export type AoldaEmptySize = keyof typeof AOLDA_EMPTY_VARIANTS.size;

export interface AoldaEmptyVariantsProps {
  /**
...
export function emptyVariants({
  size = AOLDA_EMPTY_DEFAULT_VARIANTS.size,
}: AoldaEmptyVariantsProps = {}) {
  return cn(
    "flex w-full flex-col items-center rounded-xl border border-aolda-fill bg-aolda-control text-aolda-default",
    AOLDA_EMPTY_VARIANTS.size[size].classes,
  );
}
 ...
 *   commandLine="npm install @aolda/ui"
 * />
 * ```
 */
export interface EmptyProps extends AoldaEmptyVariantsProps {
...
export function Empty({
  ...
  return (
    <div className={cn(emptyVariants({ size }), className)}>
      ...
      {description && (
        <p className="max-w-140 text-center text-aolda-strong">{description}</p>
      )}
      ...
      {commandLine && (
        <div
          className={cn(
            ...
            "bg-aolda-overlay pr-2 pl-3",
            "transition-all duration-300 hover:border-aolda-interact/80 hover:shadow-md",
            "border border-aolda-fill/60",
          )}
        >
          <span className="text-xs text-aolda-inactive select-none">$</span>
          <span className="no-scrollbar overflow-scroll text-[14px] whitespace-nowrap text-aolda-brand">
            {commandLine}
          </span>
          <Button
            className="group"
          ...
          >
            {emptyStateCopied ? (
              <CheckIcon
                size={16}
                className="animate-bounce-in text-aolda-success"
              />
            ) : (
              <CopyIcon
                size={16}
                className="text-aolda-inactive group-hover:text-aolda-brand"
              />
            )}
          </Button>
        </div>
