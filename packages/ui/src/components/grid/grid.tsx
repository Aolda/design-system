...
export const AOLDA_GRID_VARIANTS = {
  variant: {
    "2up": {
...
export const AOLDA_GRID_DEFAULT_VARIANTS = {
  gap: "base",
} as const;

export type AoldaGridVariant = keyof typeof AOLDA_GRID_VARIANTS.variant;
export type AoldaGridGap = keyof typeof AOLDA_GRID_VARIANTS.gap;
...
export function gridVariants({
  variant,
  gap = AOLDA_GRID_DEFAULT_VARIANTS.gap,
}: {
  variant?: AoldaGridVariant;
  gap?: AoldaGridGap;
} = {}) {
  return cn(
    "grid",
    variant && AOLDA_GRID_VARIANTS.variant[variant].classes,
    AOLDA_GRID_VARIANTS.gap[gap].classes,
  );
}
...
export function gridItemVariants({
  ...
  return cn(
    mobileDivider &&
      variant === "4up" &&
      "border-b border-aolda-hairline pb-8 md:border-b-0 md:pb-0",
  );
}
