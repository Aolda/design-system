...
export const AOLDA_COLLAPSIBLE_VARIANTS = {} as const;

export const AOLDA_COLLAPSIBLE_DEFAULT_VARIANTS = {} as const;

export interface AoldaCollapsibleVariantsProps {}

export function collapsibleVariants(
  _props: AoldaCollapsibleVariantsProps = {},
) {
  return cn();
}
 ...
 *
 * Use this for quick migration or when you want the default Aolda style.
 *
 * @example
 * ```tsx
 * <Collapsible.Root>
  ...
  return (
    <CollapsibleBase.Trigger
      ...
      className={cn(
        ...
        "flex cursor-pointer items-center gap-1 text-sm text-aolda-link select-none",
        className,
      )}
    >
      {children}{" "}
...
>(({ children, className }, ref) => {
  return (
    <CollapsibleBase.Panel
      ref={ref}
      className={cn(
        "my-2 space-y-4 border-l-2 border-aolda-fill pl-4",
        className,
      )}
    >
      {children}
 ...
 *
 * Use `DefaultTrigger` and `DefaultPanel` for the classic Aolda style:
 *
 * ```tsx
 * <Collapsible.Root>
 *   <Collapsible.DefaultTrigger>Show details</Collapsible.DefaultTrigger>
