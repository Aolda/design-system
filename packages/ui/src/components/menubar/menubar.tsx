...
export const AOLDA_MENUBAR_VARIANTS = {
  // MenuBar currently has no variant options but structure is ready for future additions
} as const;

export const AOLDA_MENUBAR_DEFAULT_VARIANTS = {} as const;

// Derived types from AOLDA_MENUBAR_VARIANTS
export interface AoldaMenuBarVariantsProps {}

export function menuBarVariants(_props: AoldaMenuBarVariantsProps = {}) {
  return cn(
    // Base styles
    "flex rounded-lg border border-aolda-recessed bg-aolda-recessed pl-px shadow-xs transition-colors",
  );
}
...
const MenuOption = ({
  ...
  const button = (
    <button
      aria-label={tooltip}
      className={cn(
        "relative -ml-px flex h-full w-11 cursor-pointer items-center justify-center rounded-md border-none bg-aolda-recessed first:rounded-l-lg last:rounded-r-lg transition-colors focus:z-3 focus:outline-none focus:ring-aolda-focus/50 focus-visible:z-3 focus-visible:ring-2 focus-visible:ring-aolda-brand",
        {
          "z-2 bg-aolda-base shadow-xs transition-colors": isActive === id,
        },
      )}
      onClick={onClick}
    >
...
export const MenuBar = ({
  ...
  return (
    <nav
      className={cn(
        "isolate flex rounded-lg ring ring-aolda-line bg-aolda-recessed pl-px shadow-xs transition-colors",
        className,
      )}
      ref={menuRef}
    >
