...
/**
 * CommandPalette - A composable command palette component for Aolda
 *
  ...
  return (
    <DialogBase.Root open={open} onOpenChange={onOpenChange} modal>
      <DialogBase.Portal container={container}>
        <DialogBase.Backdrop
          className="fixed inset-0 bg-aolda-overlay opacity-80 transition-all duration-150 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0"
...
function InputHeader({
  ...
  return (
    <div className="flex items-center gap-3 bg-aolda-base px-4 py-3 focus-within:ring-2 focus-within:ring-aolda-brand">
      {leading ?? (
        <MagnifyingGlassIcon
          className="h-4 w-4 text-aolda-subtle"
          weight="bold"
        />
      )}
      {children}
...
>(function List({ children, className }, ref) {
  return (
    <div
      ref={ref}
      className={cn(
        "min-h-0 flex-1 overflow-y-auto rounded-b-lg bg-aolda-base px-2 py-2 ring-1 ring-aolda-hairline",
        className,
      )}
    >
      {children}
...
function GroupLabel({
  ...
  return (
    <Autocomplete.GroupLabel
      className={cn(
        "mb-2 px-2 pt-1 text-xs font-semibold text-aolda-strong",
        className,
      )}
    >
      {children}
  ...
  return (
    <Autocomplete.Item
      ...
      className={cn(
        ...
        "cursor-pointer data-[highlighted]:bg-aolda-overlay",
        "rounded-lg",
  ...
  return (
    <Autocomplete.Empty>
      <div className="p-8 text-center">
        <p className="text-aolda-strong">{children ?? "No results found"}</p>
      </div>
    </Autocomplete.Empty>
  );
}
 ...
 *
 * Centered loading spinner using Aolda Loader.
 */
...
function Footer({ children }: CommandPaletteFooterProps) {
  return (
    <div className="flex items-center justify-between rounded-b-lg bg-aolda-elevated px-4 py-3 text-xs text-aolda-strong">
      {children}
    </div>
  );
}
...
function HighlightedText({
  ...
  mergedHighlights.forEach((range, i) => {
    ...
    parts.push(
      <mark
        key={`highlight-${i}`}
        className="rounded-sm bg-aolda-warning/50 text-aolda-default"
      >
...
function ResultItem<T>({
  ...
  return (
    ...
    >
      {icon && (
        <div className="flex flex-shrink-0 items-center text-aolda-subtle">
          {icon}
        </div>
      )}
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 truncate">
          {breadcrumbs?.map((crumb, index) => (
            <span key={index} className="flex items-center gap-2">
              <HighlightedText
                ...
                className="text-base text-aolda-default"
              />
              <CaretRightIcon
                className="h-3 w-3 flex-shrink-0 text-aolda-subtle"
                weight="bold"
              />
            </span>
          ))}
          ...
          />
          {external && (
            <ArrowSquareOutIcon className="h-3.5 w-3.5 flex-shrink-0 text-aolda-subtle" />
          )}
          {description && (
            <>
              <span className="text-aolda-strong">—</span>
              <span className="truncate text-sm text-aolda-strong">
                {description}
              </span>
            </>
          )}
      ...
      {showArrow && !external && !nonInteractive && (
        <ArrowRightIcon className="h-4 w-4 flex-shrink-0 text-aolda-subtle opacity-0 transition-opacity group-data-[highlighted]:opacity-100" />
      )}
    </Autocomplete.Item>
  );
}
...
function Container({
  ...
  return (
    <div
      className={cn(
        "flex max-h-[60vh] flex-col overflow-hidden rounded-lg bg-aolda-elevated",
        className,
      )}
    >
      {children}
  ...
  return (
    <InputHeader leading={leading} trailing={trailing}>
      <Autocomplete.Input
        ...
        className={cn(
          "flex-1 border-none bg-transparent text-base aolda-input-placeholder",
          "outline-none",
          className,
        )}
...
export const AOLDA_COMMAND_PALETTE_VARIANTS = {} as const;

export const AOLDA_COMMAND_PALETTE_DEFAULT_VARIANTS = {} as const;
