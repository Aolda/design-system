...
export const AOLDA_BUTTON_VARIANTS = {
  ...
  variant: {
    primary: {
      classes:
        "bg-aolda-brand !text-white hover:bg-aolda-brand-hover disabled:bg-aolda-brand/50",
      description: "High-emphasis button for primary actions",
    },
    secondary: {
      classes:
        "bg-aolda-base !text-aolda-default ring not-disabled:hover:border-aolda-line! not-disabled:hover:bg-aolda-tint disabled:bg-aolda-base/50 disabled:!text-aolda-default/70 ring-aolda-hairline data-[state=open]:bg-aolda-base",
      description: "Default button style for most actions",
    },
    ghost: {
      classes: "text-aolda-default hover:bg-aolda-tint shadow-none bg-inherit",
      description: "Minimal button with no background",
    },
    destructive: {
      classes: "bg-aolda-danger !text-white hover:bg-aolda-danger/70",
      description: "Danger button for destructive actions like delete",
    },
    "secondary-destructive": {
      classes:
        "bg-aolda-base !text-aolda-danger ring not-disabled:hover:border-aolda-line! not-disabled:hover:bg-aolda-base disabled:bg-aolda-base/50 disabled:!text-aolda-danger/70 ring-aolda-hairline data-[state=open]:bg-aolda-base",
      description:
        "Secondary button with destructive text for less prominent dangerous actions",
    },
    outline: {
      classes: "bg-transparent text-aolda-default ring ring-aolda-hairline",
...
// Derived types from AOLDA_BUTTON_VARIANTS
export type AoldaButtonShape = keyof typeof AOLDA_BUTTON_VARIANTS.shape;
...
export interface AoldaButtonVariantsProps {
  /**
   * Button shape.
   ...
   * @default "secondary"
   */
  variant?: AoldaButtonVariant;
}

export function buttonVariants({
  variant = AOLDA_BUTTON_DEFAULT_VARIANTS.variant,
  size = AOLDA_BUTTON_DEFAULT_VARIANTS.size,
  shape = AOLDA_BUTTON_DEFAULT_VARIANTS.shape,
}: AoldaButtonVariantsProps = {}) {
  ...
  return cn(
    ...
    "border-0 shadow-xs",
    "focus:outline-none focus:ring-aolda-focus/50 focus-visible:ring-2 focus-visible:ring-aolda-brand",
    "cursor-pointer",
    // Disabled state
    "disabled:cursor-not-allowed disabled:text-aolda-subtle",
    // Apply variant, size, shape styles from AOLDA_BUTTON_VARIANTS
    AOLDA_BUTTON_VARIANTS.variant[variant].classes,
    AOLDA_BUTTON_VARIANTS.size[size].classes,
    AOLDA_BUTTON_VARIANTS.shape[shape].classes,
    isCompactShape && AOLDA_BUTTON_VARIANTS.compactSize[size].classes,
  );
}
