...
import { inputVariants, AOLDA_INPUT_VARIANTS } from "../input/input";
import { cn } from "../../utils/cn";
import { Field, type FieldErrorMatch } from "../field/field";

/** Autocomplete variant definitions. */
export const AOLDA_AUTOCOMPLETE_VARIANTS = {
  size: AOLDA_INPUT_VARIANTS.size,
} as const;

export const AOLDA_AUTOCOMPLETE_DEFAULT_VARIANTS = {
  size: "base",
} as const;

// Derived types from AOLDA_AUTOCOMPLETE_VARIANTS
export type AoldaAutocompleteSize =
  keyof typeof AOLDA_AUTOCOMPLETE_VARIANTS.size;

export interface AoldaAutocompleteVariantsProps {
  /**
   ...
   * @default "base"
   */
  size?: AoldaAutocompleteSize;
}

export function autocompleteVariants({
  size = AOLDA_AUTOCOMPLETE_DEFAULT_VARIANTS.size,
}: AoldaAutocompleteVariantsProps = {}) {
  return cn(AOLDA_INPUT_VARIANTS.size[size].classes);
}
        ...
        <AutocompleteBase.Popup
          className={(state: AutocompleteBase.Popup.State) =>
            cn(
              ...
              "bg-aolda-control text-aolda-default",
...
function Item({ children, ...props }: AutocompleteBase.Item.Props) {
  return (
    <AutocompleteBase.Item
      {...props}
      className="group mx-1.5 grid cursor-pointer grid-cols-[1fr_16px] gap-2 rounded px-2 py-1.5 text-base data-highlighted:bg-aolda-overlay data-selected:font-medium"
    >
...
function Group(props: AutocompleteBase.Group.Props) {
  return (
    <AutocompleteBase.Group
      {...props}
      className="border-t border-aolda-line mt-2 pt-2 first:border-t-0 first:mt-0 first:pt-0"
    />
  );
}
...
function Separator(props: AutocompleteBase.Separator.Props) {
  return (
    <AutocompleteBase.Separator
      {...props}
      className={cn("mx-0 my-1 h-px bg-aolda-line", props.className)}
    />
  );
}
