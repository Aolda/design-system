...
describe("Sidebar", () => {
  ...
  it("should export variant definitions", () => {
    expect(AOLDA_SIDEBAR_VARIANTS).toBeDefined();
    expect(AOLDA_SIDEBAR_VARIANTS.variant).toHaveProperty("sidebar");
    expect(AOLDA_SIDEBAR_VARIANTS.variant).toHaveProperty("floating");
    expect(AOLDA_SIDEBAR_VARIANTS.variant).toHaveProperty("inset");
    expect(AOLDA_SIDEBAR_VARIANTS.collapsible).toHaveProperty("icon");
    expect(AOLDA_SIDEBAR_VARIANTS.collapsible).toHaveProperty("offcanvas");
    expect(AOLDA_SIDEBAR_VARIANTS.collapsible).toHaveProperty("none");
    expect(AOLDA_SIDEBAR_VARIANTS.side).toHaveProperty("left");
    expect(AOLDA_SIDEBAR_VARIANTS.side).toHaveProperty("right");
  });

  it("should export default variants", () => {
    expect(AOLDA_SIDEBAR_DEFAULT_VARIANTS).toBeDefined();
    expect(AOLDA_SIDEBAR_DEFAULT_VARIANTS.variant).toBe("sidebar");
    expect(AOLDA_SIDEBAR_DEFAULT_VARIANTS.side).toBe("left");
    expect(AOLDA_SIDEBAR_DEFAULT_VARIANTS.collapsible).toBe("icon");
  });

  it("should export styling metadata", () => {
    expect(AOLDA_SIDEBAR_STYLING).toBeDefined();
    expect(AOLDA_SIDEBAR_STYLING.width.expanded).toBe("16rem");
    expect(AOLDA_SIDEBAR_STYLING.width.icon).toBe("3rem");
  });
