...
export const AOLDA_BANNER_BASE_STYLES =
...
export const AOLDA_BANNER_VARIANTS = {
  variant: {
    default: {
      classes:
        "bg-aolda-info-tint/30 border-aolda-info/50 text-aolda-info selection:bg-aolda-info selection:text-aolda-default",
      iconClasses: "text-aolda-info",
      description: "Informational banner for general messages",
    },
    alert: {
      classes:
        "bg-aolda-warning-tint/15 border-aolda-warning/50 text-aolda-warning selection:bg-aolda-warning selection:text-aolda-default",
      iconClasses: "text-aolda-warning",
      description: "Warning banner for cautionary messages",
    },
    error: {
      classes:
        "bg-aolda-danger-tint/15 border-aolda-danger/50 text-aolda-danger selection:bg-aolda-danger selection:text-aolda-default",
      iconClasses: "text-aolda-danger",
      description: "Error banner for critical issues",
    },
  },
} as const;

export const AOLDA_BANNER_DEFAULT_VARIANTS = {
  variant: "default",
} as const;

// Derived types from AOLDA_BANNER_VARIANTS
export type AoldaBannerVariant = keyof typeof AOLDA_BANNER_VARIANTS.variant;

export interface AoldaBannerVariantsProps {
  /**
   ...
   * @default "default"
   */
  variant?: AoldaBannerVariant;
}

export function bannerVariants({
  variant = AOLDA_BANNER_DEFAULT_VARIANTS.variant,
}: AoldaBannerVariantsProps = {}) {
  return cn(
    // Base styles exported for registry consumers.
    AOLDA_BANNER_BASE_STYLES,
    // Apply variant styles from AOLDA_BANNER_VARIANTS
    AOLDA_BANNER_VARIANTS.variant[variant].classes,
  );
}
...
export const Banner = forwardRef<HTMLDivElement, BannerProps>(function Banner(
  ...
  const variantConfig = AOLDA_BANNER_VARIANTS.variant[variant];
