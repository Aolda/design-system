...
export const AOLDA_TABS_VARIANTS = {
  variant: ["segmented", "underline"],
} as const;

export const AOLDA_TABS_DEFAULT_VARIANTS = {
  variant: "segmented",
} as const;

export const AOLDA_TABS_STYLING = {
  container: {
    height: 34,
    borderRadius: 8,
...
// Derived types from AOLDA_TABS_VARIANTS
export interface AoldaTabsVariantsProps {
  /**
   * Tab style.
   ...
   * @default "segmented"
   */
  variant?: (typeof AOLDA_TABS_VARIANTS.variant)[number];
}
 ...
 * ```
 */
export type TabsProps = AoldaTabsVariantsProps & {
...
export function Tabs({
  ...
  listClassName,
  indicatorClassName,
  variant = AOLDA_TABS_DEFAULT_VARIANTS.variant,
}: TabsProps) {
  ...
  return (
    ...
    >
      ...
      {isSegmented && (
        <div className="absolute inset-x-0 top-1/2 z-0 h-9 -translate-y-1/2 rounded-lg bg-aolda-recessed" />
      )}
      <TabsPrimitive.List
        ...
        className={cn(
          ...
          isSegmented &&
            "h-9 rounded-lg bg-aolda-recessed px-0.5 ring ring-aolda-hairline/70",
          isUnderline && "h-7 gap-4 border-b border-aolda-hairline pb-2",
          listClassName,
        )}
      >
        {items.map((tab) => (
          <TabsPrimitive.Tab
            ...
            render={tab.render}
            className={cn(
              "relative z-2 flex cursor-pointer items-center rounded bg-transparent text-base whitespace-nowrap focus:outline-none focus:ring-aolda-focus/50 focus-visible:ring-2 focus-visible:ring-aolda-brand",
              isSegmented &&
                "my-0.5 rounded-md px-2.5 text-aolda-strong hover:text-aolda-default aria-selected:text-aolda-default focus-visible:ring-inset",
              isUnderline &&
                "px-2 py-2.5 text-aolda-strong hover:bg-aolda-tint hover:text-aolda-subtle aria-selected:hover:bg-aolda-tint aria-selected:font-medium aria-selected:text-aolda-default",
              tab.className,
            )}
          >
            {tab.label}
          ...
          render={(props) => (
            <div
              ...
              className={cn(
                ...
                isSegmented &&
                  "top-(--active-tab-top) h-(--active-tab-height) rounded-md bg-aolda-base shadow-sm ring ring-aolda-line",
                isUnderline && "bottom-0 h-0.5 bg-aolda-brand",
                indicatorClassName,
              )}
            />
          )}
