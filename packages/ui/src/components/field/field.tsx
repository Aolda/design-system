...
export const AOLDA_FIELD_VARIANTS = {
  // Field currently has no variant options but structure is ready for future additions
} as const;

export const AOLDA_FIELD_DEFAULT_VARIANTS = {} as const;

// Derived types from AOLDA_FIELD_VARIANTS
export interface AoldaFieldVariantsProps {
  /**
 ...
 * ```
 */
export interface FieldProps extends AoldaFieldVariantsProps {
...
export function Field({
  ...
  return (
    <FieldBase.Root className={fieldVariants({ controlFirst })}>
      <FieldBase.Label className="m-0 text-base font-medium text-aolda-default">
      ...
      {error ? (
        <FieldBase.Error
          className={cn(
            "text-sm leading-snug text-aolda-danger",
        ...
        description && (
          <FieldBase.Description
            className={cn(
              "text-sm leading-snug text-aolda-subtle",
              // Span full width in horizontal layout
              "col-span-full",
            )}
          >
