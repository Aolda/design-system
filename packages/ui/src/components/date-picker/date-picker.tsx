...
/** DatePicker variant definitions mapping tone names to their root classes. */
export const AOLDA_DATE_PICKER_VARIANTS = {
  variant: {
    primary: {
      classes: "aolda-datepicker-primary",
      description: "Brand-colored date selection states",
    },
    neutral: {
      classes: "aolda-datepicker-neutral",
      description: "Neutral high-contrast date selection states",
    },
  },
} as const;

export const AOLDA_DATE_PICKER_DEFAULT_VARIANTS = {
  variant: "primary",
} as const;

export type AoldaDatePickerVariant =
  keyof typeof AOLDA_DATE_PICKER_VARIANTS.variant;

export interface AoldaDatePickerVariantsProps {
  /**
   * Visual tone for selected dates and range endpoints.
   * - `"primary"` — Uses the brand color for selected states
   * - `"neutral"` — Uses neutral contrast colors for selected states
   * @default "primary"
   */
  variant?: AoldaDatePickerVariant;
}

export function datePickerVariants({
  variant = AOLDA_DATE_PICKER_DEFAULT_VARIANTS.variant,
}: AoldaDatePickerVariantsProps = {}) {
  return cn(
    "rdp-root select-none rounded-xl bg-aolda-base p-3 pt-1",
    AOLDA_DATE_PICKER_VARIANTS.variant[variant].classes,
  );
}
 ...
 * Uses `onChange` instead of `onSelect` for Aolda consistency.
 ...
 *
 * Built on [react-day-picker](https://daypicker.dev) with Aolda styling.
...
export function DatePicker({
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...classNames,
        root: cn(datePickerVariants({ variant }), classNames?.root, className),
      }}
      components={{
        Chevron,
        ...props.components,
