...
export function CodeHighlighted({
  ...
  const handleCopy = useCallback(async () => {
    try {
      ...
      console.error(
        "[Aolda CodeHighlighted] Failed to copy to clipboard:",
        err,
      );
    }
  }, [code]);
  ...
  const containerClasses = cn(
    "group relative m-0 w-full min-w-0 rounded-md border border-aolda-fill bg-aolda-base p-0",
  ...
  const lineNumbers =
    showLineNumbers && !isSingleLine ? (
      <div
        className="aolda-line-numbers shrink-0 select-none py-4 pr-4 text-right font-mono text-sm opacity-40"
        aria-hidden="true"
      >
  ...
  if (error) {
    console.error("[Aolda CodeHighlighted] Shiki initialization error:", error);
  }
    ...
    return (
      <div className={containerClasses}>
        {lineNumbers && (
          <div className="flex">
            {lineNumbers}
            <pre className="!m-0 min-w-0 flex-1 overflow-x-auto !p-4 font-mono text-sm leading-relaxed text-aolda-strong">
      ...
      {lineNumbers && (
        <div className="flex w-full">
          ...
          <div className="min-w-0 flex-1 overflow-x-auto">
            <div
              className="aolda-shiki [&>pre]:!m-0 [&>pre]:!border-0 [&>pre]:!rounded-none [&>pre]:!bg-transparent [&>pre]:!p-4 [&>pre]:font-mono [&>pre]:text-sm [&>pre]:leading-relaxed [&_code]:!m-0 [&_code]:!p-0 [&_code]:!bg-transparent [&_code]:!border-0"
