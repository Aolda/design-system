 ...
 * @example
 * ```tsx
 * import { useShikiHighlighter } from "@aolda/ui/code";
 *
  ...
  if (!context) {
    throw new Error(
      "useShikiHighlighter must be used within a ShikiProvider. " +
        "Wrap your app with <ShikiProvider> from '@aolda/ui/code'.",
    );
  }
  ...
  const highlight = useCallback(
    (code: string, lang: SupportedLanguage): string | null => {
      ...
      if (!languages.includes(lang)) {
        console.warn(
          `[Aolda CodeHighlighted] Language "${lang}" is not in the ShikiProvider's languages list. ` +
      ...
      try {
        ...
        console.warn(
          `[Aolda CodeHighlighted] Failed to highlight code with language "${lang}":`,
          err,
        );
        return null;
      }
