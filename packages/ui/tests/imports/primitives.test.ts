...
const EXCLUDED_EXPORT_PREFIXES = ["./internals/"];

function isIncludedBaseUiExport(key: string) {
  return (
    key.startsWith("./") &&
    !EXCLUDED_EXPORTS.has(key) &&
    !EXCLUDED_EXPORT_PREFIXES.some((prefix) => key.startsWith(prefix))
  );
}
...
describe("Primitives Export", () => {
  ...
  describe("Sync with base-ui", () => {
    ...
    it("should export all base-ui primitives from aolda", async () => {
      ...
      // Import aolda primitives
      const aoldaPrimitives = await import("../../src/primitives/index.ts");
      const aoldaExportNames = Object.keys(aoldaPrimitives);

      // For each base-ui export, check that aolda re-exports something from it
      ...
      for (const exportName of baseUiExports) {
        ...
        if (!aoldaExportNames.includes(pascalName)) {
      ...
      if (missingExports.length > 0) {
        console.error(
          "\n❌ Base-ui primitives missing from @aolda/ui/primitives:",
        );
